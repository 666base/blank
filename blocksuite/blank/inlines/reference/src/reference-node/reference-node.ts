...
import { blankTextStyles } from '@blocksuite/blank-shared/styles';
...
export class BlankReference extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    ...
    .blank-reference:hover {
      background: var(--blank-hover-color);
    }

    .blank-reference[data-selected='true'] {
      background: var(--blank-hover-color);
    }

    .blank-reference-title {
      margin-left: 4px;
      border-bottom: 0.5px solid var(--blank-divider-color);
      transition: border 0.2s ease-out;
    }
    .blank-reference-title:hover {
      border-bottom: 0.5px solid var(--blank-icon-color);
    }
  `;
  ...
  get inlineEditor() {
    const inlineRoot = this.closest<InlineRootElement<BlankTextAttributes>>(
      `[${INLINE_ROOT_ATTR}]`
    );
  ...
  override render() {
    ...
    const style = blankTextStyles(
      attributes,
      isDeleted
        ? {
            color: 'var(--blank-text-disable-color)',
            textDecoration: 'line-through',
            fill: 'var(--blank-text-disable-color)',
          }
        : {}
    );
  ...
  accessor delta: DeltaInsert<BlankTextAttributes> = {
