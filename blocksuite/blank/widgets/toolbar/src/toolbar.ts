import { DatabaseSelection } from '@blocksuite/blank-block-database';
import { EdgelessLegacySlotIdentifier } from '@blocksuite/blank-block-surface';
import { TableSelection } from '@blocksuite/blank-block-table';
import {
  darkToolbarStyles,
...
export const BLANK_TOOLBAR_WIDGET = 'blank-toolbar-widget';

export class BlankToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'blank:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'blank:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('blank:surface:frame')) {
          sideOptions = sideMap.get('blank:surface:frame') ?? null;
        } else if (flavours.has('blank:surface:group')) {
          sideOptions = sideMap.get('blank:surface:group') ?? null;
        }
      }
    }
    ...
    if (IS_MOBILE) {
      ...
      const syncMobileTextSelection = () => {
        ...
        batch(() => {
          ...
          flavour$.value = 'blank:note';
          placement$.value = toolbarRegistry.getModulePlacement('blank:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'blank:note' ? 'top' : 'top-start'
          );
