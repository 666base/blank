  ...
  getPreviewStd = (blockIds: string[]) => {
    ...
    const extensions = [
      ...
      EditorSettingExtension({ setting$: settingSignal }),
      {
        setup(di) {
          di.override(
            BlockViewIdentifier('blank:database'),
            () => literal`blank-dnd-preview-database`
          );
        },
      } as ExtensionType,
      {
        setup(di) {
          di.override(BlockViewIdentifier('blank:image'), () => {
            return (model: BlockModel) => {
              const parent = model.store.getParent(model.id);

              if (parent?.flavour === 'blank:surface') {
                return literal`blank-edgeless-placeholder-preview-image`;
              }

              return literal`blank-placeholder-preview-image`;
            };
          });
        },
      } as ExtensionType,
    ...
    const noteBlock = this.widget.host.querySelector('blank-note');
