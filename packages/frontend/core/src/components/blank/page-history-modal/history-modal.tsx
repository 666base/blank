import { Avatar, Loading, Scrollable } from '@blank/component';
import { EditorLoading } from '@blank/component/page-detail-skeleton';
import { Button, IconButton } from '@blank/component/ui/button';
import { Modal, useConfirmModal } from '@blank/component/ui/modal';
...
import type { Store, Workspace } from '@blocksuite/blank/store';
...
import { BlankErrorBoundary } from '../blank-error-boundary';
import {
    ...
    return (
      <div className={styles.previewContent}>
        ...
        {snapshotPage ? (
          <BlankErrorBoundary>
            <Scrollable.Root>
              <Scrollable.Viewport className="blank-page-viewport">
...
const PlanPrompt = () => {
      ...
      <div className={styles.planPromptTitle}>
        {
          isProWorkspace !== null
            ? !isProWorkspace
              ? t[
                  'com.blank.history.confirm-restore-modal.plan-prompt.limited-title'
                ]()
              : t[
                  'com.blank.history.confirm-restore-modal.plan-prompt.title'
                ]()
  ...
  const planDescription = useMemo(() => {
    if (!isProWorkspace) {
      return (
        <>
          <Trans i18nKey="com.blank.history.confirm-restore-modal.free-plan-prompt.description">
          ...
          {isOwner ? (
            ...
            >
              {t[
                'com.blank.history.confirm-restore-modal.pro-plan-prompt.upgrade'
              ]()}
            </span>
          ) : null}
        </>
      ...
      return (
        <Trans i18nKey="com.blank.history.confirm-restore-modal.pro-plan-prompt.description">
  ...
  return (
    <div className={styles.historyList}>
      <div className={styles.historyListHeader}>
        {t['com.blank.history.version-history']()}
      </div>
                    ...
                    return (
                      <Fragment key={history.timestamp}>
                        ...
                        {isLastGroup && isLastItem && onLoadMore ? (
                          ...
                          >
                            {t[
                              'com.blank.history.confirm-restore-modal.load-more'
                            ]()}
                          </Button>
                        ) : null}
                      </Fragment>
  ...
  return (
    ...
    >
      ...
      <div className={styles.emptyHistoryPromptTitle}>
        {t['com.blank.history.empty-prompt.title']()}
      </div>
      <div className={styles.emptyHistoryPromptDescription}>
        {t['com.blank.history.empty-prompt.description']()}
      </div>
    </div>
  );
};
...
const PageHistoryManager = ({
  ...
  const onConfirmRestore = useCallback(() => {
    openConfirmModal({
      title: t['com.blank.history.restore-current-version'](),
      description: t['com.blank.history.confirm-restore-modal.hint'](),
      cancelText: t['Cancel'](),
      contentOptions: {
        ['data-testid' as string]: 'confirm-restore-history-modal',
        style: { padding: '20px 26px' },
      },
      confirmText: t['com.blank.history.confirm-restore-modal.restore'](),
    ...
    <div className={styles.root}>
      ...
      <div className={styles.historyFooter}>
        <Button onClick={onClose}>
          {t['com.blank.history.back-to-page']()}
        </Button>
        ...
        >
          {t['com.blank.history.restore-current-version']()}
        </Button>
      </div>
    </div>
  );
