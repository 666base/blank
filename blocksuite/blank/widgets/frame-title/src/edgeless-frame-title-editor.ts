import { FrameBlockModel } from '@blocksuite/blank-model';
...
import {
  ...
  type BlankFrameTitleWidget,
} from './blank-frame-title-widget';
import type { BlankFrameTitle } from './frame-title';
...
export class EdgelessFrameTitleEditor extends WithDisposable(
  ...
  override render() {
    ...
    const frameTitleWidget = this.edgeless.std.view.getWidget(
      BLANK_FRAME_TITLE_WIDGET,
      this.frameModel.id
    ) as BlankFrameTitleWidget | null;

    if (!frameTitleWidget) return nothing;

    const frameTitle =
      frameTitleWidget.querySelector<BlankFrameTitle>('blank-frame-title');

    const colors = frameTitle?.colors ?? {
      background: cssVarV2('edgeless/frame/background/white'),
      text: 'var(--blank-text-primary-color)',
    };
