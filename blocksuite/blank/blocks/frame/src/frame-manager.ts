import type { SurfaceBlockModel } from '@blocksuite/blank-block-surface';
import { Overlay } from '@blocksuite/blank-block-surface';
...
export class EdgelessFrameManager extends GfxExtension {
  ...
  private _addFrameBlock(bound: Bound) {
    ...
    const id = this.gfx.doc.addBlock('blank:frame', props, surfaceModel);
...
export function isFrameBlock(element: unknown): element is FrameBlockModel {
  return !!element && (element as BlockModel).flavour === 'blank:frame';
}
