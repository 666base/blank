...
import { focusTextModel, type RichText } from '@blocksuite/blank-rich-text';
...
export class DocTitle extends WithDisposable(ShadowlessElement) {
  ...
  private _getOrCreateFirstPageVisibleNote() {
    ...
    const noteId = this.doc.addBlock('blank:note', {}, this._rootModel, 0);
  ...
  private get _viewport() {
    return (
      this.closest<HTMLElement>('.blank-page-viewport') ??
      this.closest<HTMLElement>('.blank-edgeless-viewport')
    );
  }
