import { deleteTextCommand } from '@blocksuite/blank-inline-preset';
...
export class CodeBlockClipboardController extends LifeCycleWatcher {
  ...
  onPaste: UIEventHandler = ctx => {
    ...
    const codeBlock = selectedBlockId
      ...
      : null;
    if (plainText && codeBlock?.flavour === 'blank:code' && selectedBlockId) {
  ...
  override mounted() {
    ...
    const subscription = this.std.view.viewUpdated.subscribe(
      ({ type, method, view }) => {
        if (type !== 'block' || view.model.flavour !== 'blank:code') return;
