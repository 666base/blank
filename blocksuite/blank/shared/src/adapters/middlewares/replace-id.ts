  ...
  ({ slots, docCRUD, assetsManager }) => {
    ...
    const replaceSurfaceRefIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        // Otherwise,
        // 1. If the reference is an blank:frame not in doc, generate a new id.
        // 2. If the reference is graph, keep the original id.
        if (idMap.has(original)) {
          model.props.reference = idMap.get(original)!;
        } else if (
          model.props.refFlavour === 'blank:frame' &&
    ...
    const beforeImportBlockSubscription = slots.beforeImport
      ...
      .subscribe(payload => {
        ...
        if (snapshot.flavour === 'blank:page') {
          const index = snapshot.children.findIndex(
            c => c.flavour === 'blank:surface'
          );
          if (index !== -1) {
        ...
        // Should be re-paired.
        if (['blank:attachment', 'blank:image'].includes(snapshot.flavour)) {
        ...
        if (snapshot.flavour === 'blank:surface') {
