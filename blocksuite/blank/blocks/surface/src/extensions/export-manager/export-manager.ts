import { ImageBlockModel, type RootBlockModel } from '@blocksuite/blank-model';
import { FetchUtils } from '@blocksuite/blank-shared/adapters';
import {
  CANVAS_EXPORT_IGNORE_TAGS,
  DEFAULT_IMAGE_PROXY_ENDPOINT,
} from '@blocksuite/blank-shared/consts';
import type { Viewport } from '@blocksuite/blank-shared/types';
import {
  isInsidePageEditor,
  matchModels,
    ...
    const html2canvasOption = {
      ignoreElements: function (element: Element) {
        if (
          ...
          // the close and expand buttons in blank-doc-meta-data is not needed to be showed
          return true;
        } else {
          return false;
        }
  ...
  private async _html2canvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async (documentClone: Document, element: HTMLElement) => {
        ...
        const layer = element.classList.contains('.blank-edgeless-layer')
          ? element
          : null;
