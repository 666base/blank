import { appendParagraphCommand } from '@blocksuite/blank-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
import { focusTextModel } from '@blocksuite/blank-rich-text';
import {
  PageViewportService,
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  static override styles = css`
    editor-host:has(> blank-page-root, * > blank-page-root) {
      display: block;
      height: 100%;
    }
    ...
    .blank-page-root-block-container {
      ...
      font-weight: 400;
      max-width: var(--blank-editor-width);
      margin: 0 auto;

      /* Leave a place for drag-handle */
      /* Do not use prettier format this style, or it will be broken */
      /* prettier-ignore */
      padding-left: var(--blank-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-right: var(--blank-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-bottom: var(--blank-editor-bottom-padding, ${DOC_BOTTOM_PADDING}px);
    }
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('blank:note', {}, store.root?.id);
  ...
  private _getDefaultNoteBlock() {
    return (
      this.store.root?.children.find(
        child => child.flavour === 'blank:note'
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        if (!prevNote || prevNote.flavour !== 'blank:note') {
        ...
        const notes = this.store.getModelsByFlavour('blank:note');
  ...
  override renderBlock() {
    ...
    return html`
      <div class="blank-page-root-block-container">${children} ${widgets}</div>
    `;
  }

  @query('.blank-page-root-block-container')
