...
import type { BlobSyncState } from '@blank/nbstore';
    ...
    const disposableOverCapacity =
      currentWorkspace.engine.blob.state$.subscribe(
        debounce(({ overCapacity }: BlobSyncState) => {
          ...
          if (isOwner) {
            notify.warning({
              title: t['com.blank.payment.storage-limit.new-title'](),
              message:
                t['com.blank.payment.storage-limit.new-description.owner'](),
              actions: [
                {
                  key: 'upgrade',
                  label: t['com.blank.payment.upgrade'](),
            ...
            notify.warning({
              title: t['com.blank.payment.storage-limit.new-title'](),
              message:
                t['com.blank.payment.storage-limit.description.member'](),
            });
          }
        })
      );
