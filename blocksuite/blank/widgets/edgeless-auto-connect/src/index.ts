...
import { SmallDocIcon } from '@blocksuite/blank-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
import { matchModels, stopPropagation } from '@blocksuite/blank-shared/utils';
...
export const BLANK_EDGELESS_AUTO_CONNECT_WIDGET =
  'blank-edgeless-auto-connect-widget';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
    ...
    return html`${repeat(
      ...
      note => {
        ...
        const style = styleMap({
          ...
          borderRadius: '50%',
          backgroundColor: 'var(--blank-text-secondary-color)',
        ...
        return html`<div style=${style} class="edgeless-only-index-label">
          ...
          <blank-tooltip tip-position="bottom">
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('blank:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'blank:surface-ref') {
          switch (payload.type) {
            case 'add':
...
export const autoConnectWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(BLANK_EDGELESS_AUTO_CONNECT_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    'blank-edgeless-auto-connect-widget': EdgelessAutoConnectWidget;
  }
}
