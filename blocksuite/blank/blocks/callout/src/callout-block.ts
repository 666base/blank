...
import { DefaultInlineManagerExtension } from '@blocksuite/blank-inline-preset';
import {
  type CalloutBlockModel,
  type ParagraphBlockModel,
} from '@blocksuite/blank-model';
import { focusTextModel } from '@blocksuite/blank-rich-text';
import { EDGELESS_TOP_CONTENTEDITABLE_SELECTOR } from '@blocksuite/blank-shared/consts';
import {
  DocModeProvider,
  type IconData,
...
// Copy of renderUniLit and UniLit from blank-data-view
...
const getIcon = (icon?: IconData) => {
  ...
  if (icon.type === IconType.BlankIcon) {
    return (
...
export class CalloutBlockComponent extends CaptionedBlockComponent<CalloutBlockModel> {
  ...
  private _getEmojiMarginTop(): string {
    ...
    const marginTopMap: Record<string, string> = {
      'blank:paragraph:h1': '23px',
      'blank:paragraph:h2': '20px',
      'blank:paragraph:h3': '16px',
      'blank:paragraph:h4': '15px',
      'blank:paragraph:h5': '14px',
      'blank:paragraph:h6': '13px',
    };
  ...
  private _toggleIconPicker(event: MouseEvent) {
    ...
    wrapper.style.boxShadow = 'var(--blank-menu-shadow)';
  ...
  private readonly _handleBlockClick = (event: MouseEvent) => {
    ...
    if (
      target.closest('.blank-callout-emoji-container') ||
      target.classList.contains('blank-callout-emoji')
    ) {
      return;
    }
    ...
    const paragraphId = this.store.addBlock('blank:paragraph', {}, this.model);
