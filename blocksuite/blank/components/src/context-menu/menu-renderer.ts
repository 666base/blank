  ...
  implements MenuComponentInterface
{
  ...
  renderSearch() {
    ...
    return html` <div style=${searchStyle} class="blank-menu-search-container">
      <div
        style="font-size:20px;display:flex;align-items:center;color: var(--blank-text-secondary-color)"
      >
        ${SearchIcon()}
      </div>
      <input
        autocomplete="off"
        class="blank-menu-search"
    ...
    return html`
      ...
      >
        ${title.onBack
          ...
          : nothing}
        <div
          style="flex:1;font-weight:500;font-size: 14px;line-height: 22px;color: var(--blank-text-primary-color)"
        >
          ${title.text}
        </div>
        ${title.postfix?.()}
...
export const createModal = (container: HTMLElement = document.body) => {
  ...
  div.style.fontFamily = 'var(--blank-font-family)';
