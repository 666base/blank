import { getRangeByPositions } from '@blocksuite/blank-shared/utils';
...
export class TableSelectionController implements ReactiveController {
  ...
  private resolveDragStartTarget(
    ...
    if (fillValues) {
      ...
      cell = target.closest('blank-database-virtual-cell-container');
    }
  ...
  getCellElement(cell: TableGridCell): DatabaseCellContainer | undefined {
    return (
      cell.element.querySelector('blank-database-virtual-cell-container') ??
      undefined
    );
  }
...
export class SelectionElement extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .database-selection {
      ...
      background-color: var(--blank-primary-color-04);
      pointer-events: none;
      display: none;
    }
