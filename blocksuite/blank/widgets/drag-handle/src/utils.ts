import { type CalloutBlockComponent } from '@blocksuite/blank-block-callout';
import {
  BLANK_EDGELESS_NOTE,
  EdgelessNoteBackground,
  EdgelessNoteBlockComponent,
} from '@blocksuite/blank-block-note';
...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.blank-database-block-table');
};
...
export const getParentNoteBlock = (blockComponent: BlockComponent) => {
  return blockComponent.closest('blank-note, blank-edgeless-note') ?? null;
};
...
export const getClosestNoteBlock = (
  ...
  return isInsidePageEditor
    ? findClosestBlockComponent(rootComponent, point, 'blank-note')
...
export const getClosestBlockByPoint = (
  ...
  if (!closestNoteBlock || closestNoteBlock.closest('.blank-surface-ref')) {
    return null;
  }
  ...
  const blockSelector =
    '.blank-note-block-container > .blank-block-children-container > [data-block-id]';
  ...
  if (matchModels(closestBlock.model, [ParagraphBlockModel])) {
    const callout =
      closestBlock.closest<CalloutBlockComponent>('blank-callout');
    if (callout) {
      return callout;
    }
  }
...
function getHoveringNote(point: Point) {
  ...
  for (const el of elements) {
    ...
    if (el instanceof EdgelessNoteBackground) {
      return el.closest(BLANK_EDGELESS_NOTE) ?? null;
    }
  }
  return null;
}
