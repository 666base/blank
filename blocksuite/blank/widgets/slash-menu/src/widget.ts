import { getInlineEditorByModel } from '@blocksuite/blank-rich-text';
...
import { BLANK_SLASH_MENU_TRIGGER_KEY } from './consts';
...
export class BlankSlashMenuWidget extends WidgetComponent {
  ...
  private readonly _handleInput = (
    ...
    inlineRangeApplyCallback(() => {
      ...
      if (!text.endsWith(BLANK_SLASH_MENU_TRIGGER_KEY)) return;
  ...
  private readonly _onCompositionEnd = (ctx: UIEventStateContext) => {
    ...
    if (event.data !== BLANK_SLASH_MENU_TRIGGER_KEY) return;
  ...
  private readonly _onBeforeInput = (ctx: UIEventStateContext) => {
    ...
    if (!event.data.includes(BLANK_SLASH_MENU_TRIGGER_KEY)) return;
