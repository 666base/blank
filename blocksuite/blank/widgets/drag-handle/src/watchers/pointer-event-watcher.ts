import type { NoteBlockComponent } from '@blocksuite/blank-block-note';
import { captureEventTarget } from '@blocksuite/blank-shared/utils';
...
import { BLANK_DRAG_HANDLE_WIDGET } from '../consts.js';
...
export class PointerEventWatcher {
  ...
  private readonly _clickHandler: UIEventHandler = ctx => {
    ...
    const insideDragHandle = !!element?.closest(BLANK_DRAG_HANDLE_WIDGET);
  ...
  private readonly _pointerOutHandler: UIEventHandler = ctx => {
    ...
    const inPage = !!relatedElement?.closest('.blank-page-viewport');

    const inDragHandle = !!relatedElement?.closest(BLANK_DRAG_HANDLE_WIDGET);
  ...
  private readonly _throttledPointerMoveHandler = throttle<UIEventHandler>(
    ctx => {
      ...
      if (
        element.closest('.blank-drag-handle-container') ||
        element.closest('.blank-add-block-widget-container')
      ) {
        return;
      }
