import { type Notification, notify } from '@blank/component';
import {
  RouteLogic,
  useNavigateHelper,
} from '@blank/core/components/hooks/use-navigate-helper';
...
export const AIOnboardingLocal = () => {
  ...
  const actions = useMemo(() => {
    const result: NonNullable<Notification['actions']> = [
      {
        key: 'learn-more',
        label: t['com.blank.ai-onboarding.local.action-learn-more'](),
        onClick: () => {
          window.open('https://ai.blank.pro', '_blank', 'noreferrer');
        },
      },
    ];
    if (notSignedIn) {
      result.push({
        key: 'get-started',
        label: t['com.blank.ai-onboarding.local.action-get-started'](),
        onClick: () => {
      ...
      const id = notify(
        {
          title: (
            <div className={styles.title}>
              {t['com.blank.ai-onboarding.local.title']()}
            </div>
          ),
          message: t['com.blank.ai-onboarding.local.message'](),
          icon: <AiIcon />,
