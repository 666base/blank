...
import { on, once } from '@blocksuite/blank-shared/utils';
...
const styles = css`
  ...
  .frame-card-container.dragging {
    ...
    top: 0;
    left: 0;
    z-index: calc(var(--blank-z-index-popover, 0) + 3);
  }
...
export const BLANK_FRAME_CARD = 'blank-frame-card';
...
export class FrameCard extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    >
      ${this.status === 'dragging'
        ? nothing
        : html`<blank-frame-card-title
            .cardIndex=${this.cardIndex}
            .frame=${this.frame}
          ></blank-frame-card-title>`}
      <div
