...
const styles = css`
  ${unsafeCSS(dataViewCommonStyle('blank-data-view-record-detail'))}
  blank-data-view-record-detail {
    position: relative;
    display: flex;
    flex: 1;
...
export class RecordDetail extends SignalWatcher(
  ...
  override connectedCallback() {
    ...
    this.dataset.widgetId = 'blank-detail-widget';
  }
  ...
  override render() {
    ...
    return html`
      ...
      <div style="flex:1;overflow-y: auto;overflow-x: hidden">
        <div
          style="width: 100%;max-width: var(--blank-editor-width);display: flex;flex-direction: column;margin: 0 auto;box-sizing: border-box;"
        >
          ...
          ${repeat(
            ...
            property => {
              return keyed(
                this.rowId,
                html` <blank-data-view-record-field
                  .view="${this.view}"
                  .column="${property}"
                  .rowId="${this.rowId}"
                  data-column-id="${property.id}"
                ></blank-data-view-record-field>`
              );
            }
          )}
          ${!this.readonly
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-data-view-record-detail': RecordDetail;
  }
}
export const createRecordDetail = (ops: {
  ...
  return html` <blank-data-view-record-detail
    .view=${ops.view}
    .rowId=${ops.rowId}
