...
const createToastContainer = (portal?: HTMLElement) => {
  ...
  const template = html`<div
    style="${styles}"
    data-testid="blank-toast-container"
  ></div>`;
...
const createAndShowNewToast = (
  ...
  const toastStyles = css`
    ...
    background: var(--blank-tooltip);
    box-shadow: var(--blank-float-button-shadow);
    border-radius: 8px;
    opacity: 0;
  ...
  window.dispatchEvent(
    new CustomEvent('blank-toast:emit', { detail: message })
  );
