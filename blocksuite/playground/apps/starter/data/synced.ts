...
export const synced: InitFn = (collection: Workspace, id: string) => {
  ...
  docSyncedPage.load(() => {
    ...
    const rootId = docSyncedPage.addBlock('blank:page', {
      title: new Text('Synced - Page View'),
    });

    docSyncedPage.addBlock('blank:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedPage.addBlock('blank:note', {}, rootId);
  ...
  docSyncedEdgeless.load(() => {
    // Add root block and surface block at root level
    const rootId = docSyncedEdgeless.addBlock('blank:page', {
      title: new Text('Synced - Edgeless View'),
    });

    docSyncedEdgeless.addBlock('blank:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedEdgeless.addBlock('blank:note', {}, rootId);
  ...
  docMain.load(() => {
    // Add root block and surface block at root level
    const rootId = docMain.addBlock('blank:page', {
      title: new Text('Home doc, having synced blocks'),
    });

    const surfaceId = docMain.addBlock('blank:surface', {}, rootId);
    const noteId = docMain.addBlock('blank:note', {}, rootId);
