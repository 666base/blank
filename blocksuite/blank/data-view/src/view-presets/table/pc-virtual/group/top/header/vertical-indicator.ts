...
export const getTableGroupRect = (ele: HTMLElement) => {
  const group = ele.closest('blank-data-view-virtual-table-group');
  if (!group) {
    return;
  }
  ...
  const top =
    group
      .querySelector('.blank-database-column-header')
      ?.getBoundingClientRect().top ?? groupRect.top;
  const bottom =
    group.querySelector('.blank-database-block-rows')?.getBoundingClientRect()
