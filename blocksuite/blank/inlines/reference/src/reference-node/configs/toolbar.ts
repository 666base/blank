import { notifyLinkedDocSwitchedToEmbed } from '@blocksuite/blank-components/notification';
import {
  ActionPlacement,
...
import { BlankReference } from '../reference-node';
  ...
  actions: [
    {
      ...
      content(ctx) {
        ...
        return html`<blank-linked-doc-title
          .title=${originalTitle}
          .open=${open}
        ></blank-linked-doc-title>`;
      },
    },
    {
      id: 'c.conversions',
      actions: [
        ...
        },
        {
          ...
          disabled(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof BlankReference)) return true;
            ...
            if (target.closest('blank-embed-synced-doc-block')) return true;
      ...
      content(ctx) {
        ...
        return html`${keyed(
          target,
          html`<blank-view-dropdown-menu
            .actions=${actions}
            .context=${ctx}
            .onToggle=${onToggle}
            .viewTypeSignal=${viewType$}
          ></blank-view-dropdown-menu>`
        )}`;
      },
      when(ctx) {
        const target = ctx.message$.peek()?.element;
        if (!(target instanceof BlankReference)) return false;
        if (!target.block) return false;

        if (ctx.flags.isNative()) return false;
        if (
          target.block.closest('blank-database') ||
          target.block.closest('blank-table')
        )
          return false;
