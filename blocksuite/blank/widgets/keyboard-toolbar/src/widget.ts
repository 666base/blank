import { getDocTitleByEditorHost } from '@blocksuite/blank-fragment-doc-title';
...
export const BLANK_KEYBOARD_TOOLBAR_WIDGET = 'blank-keyboard-toolbar-widget';

export class BlankKeyboardToolbarWidget extends WidgetComponent<RootBlockModel> {
  ...
  get keyboard(): VirtualKeyboardProviderWithAction & { fallback?: boolean } {
    ...
    return {
      // fallback keyboard actions — never use inputMode='none', it clears in-progress text.
      fallback: true,
      show: () => {},
      hide: () => {},
      ...provider,
  ...
  override render() {
    ...
    return html`<blocksuite-portal
      .shadowDom=${false}
      .template=${html`<blank-keyboard-toolbar
...
export const keyboardToolbarWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(BLANK_KEYBOARD_TOOLBAR_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [BLANK_KEYBOARD_TOOLBAR_WIDGET]: BlankKeyboardToolbarWidget;
  }
}
