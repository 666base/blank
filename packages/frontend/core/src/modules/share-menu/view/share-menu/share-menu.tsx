import { Tabs, Tooltip, useConfirmModal } from '@blank/component';
import { Button } from '@blank/component/ui/button';
import { Menu } from '@blank/component/ui/menu';
...
export const ShareMenuContent = (props: ShareMenuProps) => {
  ...
  const openPaywallModal = useCallback(() => {
    openConfirmModal({
      title:
        t[
          `com.blank.share-menu.paywall.${isOwner ? 'owner' : 'member'}.title`
        ](),
      description:
        t[
          `com.blank.share-menu.paywall.${isOwner ? 'owner' : 'member'}.description`
        ](),
      confirmText:
        t[
          `com.blank.share-menu.paywall.${isOwner ? 'owner' : 'member'}.confirm`
        ](),
      onConfirm: onConfirm,
      cancelText: t['Cancel'](),
      cancelButtonOptions: {
...
const DefaultShareButton = forwardRef(function DefaultShareButton(
  ...
  const tooltip =
    props.tooltip ??
    (shared
      ? t['com.blank.share-menu.option.link.readonly.description']()
      : t['com.blank.share-menu.option.link.no-access.description']());
