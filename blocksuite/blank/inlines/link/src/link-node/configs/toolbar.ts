  ...
  actions: [
    {
      ...
      content(cx) {
        ...
        if (!(target instanceof BlankLink)) return null;

        const { link } = target;
        if (!link) return null;

        return html`<blank-link-preview .url=${link}></blank-link-preview>`;
      },
    },
    {
      ...
      actions: [
        {
          ...
          run(ctx) {
            ...
            if (!(target instanceof BlankLink)) return;
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          when(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof BlankLink)) return false;
      ...
      when(ctx) {
        ...
        const bookmarkSchema = schema.flavourSchemaMap.get('blank:bookmark');
