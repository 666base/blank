...
export class KanbanSelectionController implements ReactiveController {
  ...
  shiftClickCard = (event: MouseEvent) => {
    ...
    const closestGroupKey = target.closest('blank-data-view-kanban-group')
      ?.group.key;
  ...
  focusIn() {
    ...
    if (selection.selectionType === 'card') {
      ...
      const cell = card?.querySelector('blank-data-view-kanban-cell');
      if (card && cell) {
        this.selection = {
  ...
  focusNext(position: 'up' | 'down' | 'left' | 'right') {
    ...
    if (selection.selectionType === 'cell' && !selection.isEditing) {
      ...
      const group = this.host?.querySelector(
        `blank-data-view-kanban-group[data-key="${selection.cards[0].groupKey}"]`
      );
      const cardElements = Array.from(
        group?.querySelectorAll('blank-data-view-kanban-card') ?? []
      );
  ...
  getNextFocusCard(
    ...
    const groups = Array.from(
      this.host?.querySelectorAll('blank-data-view-kanban-group') ?? []
    );
  ...
  getNextFocusCell(
    ...
    const kanbanCells = getCardCellsBySelection(this.host, selection);
    const group = this.host.querySelector(
      `blank-data-view-kanban-group[data-key="${selection.groupKey}"]`
    );
...
function getNextGroupFocusElement(
  ...
  const nextCards = Array.from(
    nextGroup.querySelectorAll('blank-data-view-kanban-card')
  );
  ...
  const cells = Array.from(
    nextCard.querySelectorAll('blank-data-view-kanban-cell')
  );
  const cellPos = cells
    .map((card, index) => {
      const targetRect = card.getBoundingClientRect();
...
function getCardCellsBySelection(
  ...
  return Array.from(
    card?.querySelectorAll('blank-data-view-kanban-cell') ?? []
  );
}
...
function getSelectedCard(
  ...
  return group.querySelector<KanbanCard>(
    `blank-data-view-kanban-card[data-card-id="${selection.cardId}"]`
  );
}
  ...
  const groups = groupKeys
    .map(key =>
      viewElement.querySelector(
        `blank-data-view-kanban-group[data-key="${key}"]`
      )
    )
    ...
    .flatMap(group =>
      cardIds.map(id =>
        group.querySelector<KanbanCard>(
          `blank-data-view-kanban-card[data-card-id="${id}"]`
        )
      )
    )
...
function getFocusCell(viewElement: Element, selection: KanbanCellSelection) {
  ...
  return card?.querySelector<KanbanCell>(
    `blank-data-view-kanban-cell[data-column-id="${selection.columnId}"]`
  );
}
