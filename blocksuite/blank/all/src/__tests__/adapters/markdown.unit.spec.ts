...
import { BlankSchemas } from '../../schemas.js';
...
function simplifyBlockForSnapshot(
  ...
  if (block.flavour === 'blank:paragraph' || block.flavour === 'blank:list') {
  ...
  if (block.flavour === 'blank:callout') {
    simplified.emoji = block.props.emoji;
  }

  if (block.flavour === 'blank:attachment') {
  ...
  const children = (block.children ?? [])
    .filter(child => child.flavour !== 'blank:surface')
...
`;
  ...
  test('link', async () => {
    ...
    const markdown = 'aaa [bbb](https://blank.pro/) ccc\n';
  ...
  test('inline link', async () => {
    ...
    const markdown = 'aaa https://blank.pro/  \n';
              ...
              delta: [
                {
                  insert: 'Hello',
                  attributes: {
                    color: 'var(--blank-v2-text-highlight-fg-blue)',
                  },
                },
              ],
            },
...
`;
  ...
  test('inline link', async () => {
    const markdown = 'aaa https://blank.pro/ ccc\n';
    const blockSnapshot: BlockSnapshot = {
      type: 'block',
      id: 'matchesReplaceMap[0]',
      flavour: 'blank:note',
