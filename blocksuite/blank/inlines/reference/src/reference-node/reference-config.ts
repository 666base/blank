...
import type { BlankReference } from './reference-node';

export interface ReferenceNodeConfig {
  customContent?: (reference: BlankReference) => TemplateResult;
  interactable?: boolean;
  hidePopup?: boolean;
}

export const ReferenceNodeConfigExtension =
  ConfigExtensionFactory<ReferenceNodeConfig>('BlankReferenceNodeConfig');

export class ReferenceNodeConfigProvider {
  private _customContent:
    | ((reference: BlankReference) => TemplateResult)
    | undefined = undefined;
