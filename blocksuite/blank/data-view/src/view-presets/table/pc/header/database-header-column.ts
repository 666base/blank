...
export class DatabaseHeaderColumn extends SignalWatcher(
  ...
  static override styles = css`
    blank-database-header-column {
      display: flex;
    }

    .blank-database-header-column-grabbing * {
      cursor: grabbing;
    }
  `;
              ...
              items: [
                ...
                ...numberFormats.map(format => {
                  ...
                  return menu.action({
                    ...
                    prefix: html`<span
                      style="font-size: var(--blank-font-base); scale: 1.2;"
                      >${format.symbol}</span
                    >`,
                    name: format.label,
                    select: () => {
  ...
  override render() {
    ...
    const classes = classMap({
      'blank-database-column-move': true,
      [this.grabStatus]: true,
    });
    return html`
      <div
        style=${style}
        class="blank-database-column-content"
      ...
      >
        ...
        <div class="blank-database-column-text ${column.type$.value}">
          <div
            class="blank-database-column-type-icon dv-hover"
            @click="${this._clickTypeIcon}"
          >
            <uni-lit .uni="${column.icon}"></uni-lit>
          </div>
          <div class="blank-database-column-text-content">
            <div class="blank-database-column-text-input">
              ${column.name$.value}
            </div>
          </div>
        </div>
...
function numberFormatConfig(column: Property): MenuConfig {
  return () =>
    html` <blank-database-number-format-bar
      .column="${column}"
    ></blank-database-number-format-bar>`;
}

declare global {
  interface HTMLElementTagNameMap {
    'blank-database-header-column': DatabaseHeaderColumn;
  }
}
