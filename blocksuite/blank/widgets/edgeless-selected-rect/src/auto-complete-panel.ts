import { EdgelessFrameManagerIdentifier } from '@blocksuite/blank-block-frame';
import {
  CanvasElementType,
...
import { FontFamilyIcon } from '@blocksuite/blank-components/icons';
import {
  mountShapeTextEditor,
  SHAPE_OVERLAY_HEIGHT,
  SHAPE_OVERLAY_WIDTH,
  ShapeComponentConfig,
} from '@blocksuite/blank-gfx-shape';
import {
  insertEdgelessTextCommand,
  mountTextElementEditor,
} from '@blocksuite/blank-gfx-text';
import type {
  Connection,
...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  static override styles = css`
    .auto-complete-panel-container {
      ...
      gap: 8px;
      border-radius: 8px;
      background: var(--blank-background-overlay-panel-color);
  ...
  private _addFrame() {
    ...
    const props = this.std.get(EditPropsStore).applyLastProps('blank:frame', {
      title: new Y.Text(`Frame ${frameIndex}`),
      xywh: serializeXYWH(...xywh),
      presentationIndex: frameMgr.generatePresentationIndex(),
    });
    const id = this.crud.addBlock('blank:frame', props, surfaceBlockModel);
  ...
  private _addNote() {
    ...
    store.addBlock('blank:paragraph', { type: 'text' }, id);
  ...
  private _showFrameOverlay() {
    ...
    const strokeColor = this.std
      .get(ThemeProvider)
      .getCssVariableColor('--blank-black-30');
  ...
  private _showNoteOverlay() {
    ...
    const background = this.edgeless.std
      .get(ThemeProvider)
      .getColorValue(
        this.edgeless.std.get(EditPropsStore).lastProps$.value['blank:note']
          .background,
