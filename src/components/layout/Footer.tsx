...
export const Footer = memo(function Footer({ onOpenSettings }: FooterProps) {
  ...
  const handleCommit = useCallback(async () => {
    ...
    try {
      const success = await commit("Quick commit from Blank");
      if (success) {
  ...
  // When there's no git content and no cloud sync, show a floating settings button
  if (!hasGitFooterContent && !cloudUser) {
    return (
    ...
    <div className="shrink-0 border-t border-border">
      ...
      <div className="pl-4 pr-3 pt-2 pb-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-px">
          ...
          {cloudUser && (
            <Tooltip content="Cloud Sync Active">
              <IconButton title="Cloud Sync" className="cursor-default">
                <CloudIcon className="w-4.5 h-4.5 stroke-[1.5] text-accent" />
              </IconButton>
            </Tooltip>
          )}
