...
export function getDropRectByPoint(
  ...
  if (isDatabase) {
    ...
    for (; i < len; i++) {
      ...
      if (e.classList.contains('blank-database-block-row-cell-content')) {
      ...
      if (e.classList.contains('blank-database-block-row')) {
  ...
  if (parent?.classList.contains('blank-database-block-row-cell-content')) {
    result.flag = DropFlags.Database;
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.blank-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.blank-database-column-header');
}
...
function getDatabaseBlockRowsElement(element: Element) {
  return element.querySelector('.blank-database-block-rows');
}

function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.blank-database-block-table');
    if (!table) {
      return null;
    }
    bounds = table.getBoundingClientRect();
  }
  // blank-database-block-row-cell
