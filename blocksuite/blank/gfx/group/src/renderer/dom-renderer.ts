import { DomElementRendererExtension } from '@blocksuite/blank-block-surface';
import { FontWeight, type GroupElementModel } from '@blocksuite/blank-model';
...
export const GroupDomRendererExtension = DomElementRendererExtension(
  ...
  (model: GroupElementModel, domElement, renderer) => {
    ...
    const renderTitle = () => {
      ...
      titleElement.style.color = renderer.getPropertyValue('--blank-blue');
    ...
    if (elements.includes(model.id)) {
      if (model.showTitle) {
        ...
        domElement.style.outlineColor =
          renderer.getPropertyValue('--blank-blue');
