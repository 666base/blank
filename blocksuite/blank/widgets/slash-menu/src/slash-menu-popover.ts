...
} from '@blocksuite/blank-rich-text';
import {
  DocModeProvider,
  TelemetryProvider,
...
import {
  BLANK_SLASH_MENU_MAX_HEIGHT,
  BLANK_SLASH_MENU_TOOLTIP_TIMEOUT,
  BLANK_SLASH_MENU_TRIGGER_KEY,
} from './consts.js';
...
export class SlashMenu extends WithDisposable(LitElement) {
  ...
  private readonly _handleClickItem = (item: SlashMenuActionItem) => {
    ...
    cleanSpecifiedTail(
      this.context.std,
      this.context.model,
      BLANK_SLASH_MENU_TRIGGER_KEY + (this._query || '')
    );
    this.inlineEditor
      .waitForUpdate()
      .then(() => {
        ...
        this._telemetry?.track('SelectSlashMenuItem', {
          ...
          segment:
            this.context.model.flavour === 'blank:edgeless-text'
              ? 'edgeless-text'
              : 'doc',
          module: 'slash menu',
          control: item.name,
  ...
  constructor(
    private readonly inlineEditor: BlankInlineEditor,
  ...
  override connectedCallback() {
    ...
    createKeydownObserver({
      ...
      interceptor: (event, next) => {
        ...
        if (key === BLANK_SLASH_MENU_TRIGGER_KEY) {
  ...
  override render() {
    const slashMenuStyles = this._position
      ? {
          ...
          maxHeight: `${Math.min(this._position.height, BLANK_SLASH_MENU_MAX_HEIGHT)}px`,
        }
      : {
          visibility: 'hidden',
        };
...
export class InnerSlashMenu extends WithDisposable(LitElement) {
  ...
  private readonly _openSubMenu = (item: SlashMenuSubMenu) => {
    ...
    subMenuElement.style.zIndex = `calc(var(--blank-z-index-popover) + ${this.depth})`;
  ...
  private readonly _renderActionItem = (item: SlashMenuActionItem) => {
    ...
    >
      ...
      html`<blank-tooltip
        ...
        .hoverOptions=${{
          enterDelay: BLANK_SLASH_MENU_TOOLTIP_TIMEOUT,
          allowMultiple: false,
        }}
      >
