import { MenuItem, MenuTrigger } from '@blank/component';
...
export const FrameSettings = () => {
  ...
  const { background } = settings['blank:frame'];
  ...
  const colorItems = useMemo(() => {
    return palettes.map(({ key, value, resolvedValue }) => {
      const handler = () => {
        editorSetting.set('blank:frame', { background: value });
      };
  ...
  const getElements = useCallback((doc: Store) => {
    return doc.getBlocksByFlavour('blank:frame') || [];
  }, []);

  return (
    <>
      <EdgelessSnapshot
        title={t['com.blank.settings.editorSettings.edgeless.frame']()}
        docName="frame"
        keyName="blank:frame"
      ...
      <SettingRow
        name={t[
          'com.blank.settings.editorSettings.edgeless.frame.background'
        ]()}
        desc={''}
      >
        {currentColor ? (
