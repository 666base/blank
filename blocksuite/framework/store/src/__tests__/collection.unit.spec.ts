...
describe('basic', () => {
  ...
  it('doc ready lifecycle', () => {
    ...
    store.load(() => {
      ...
      store.addBlock('blank:note', {}, rootId);
    });
...
describe('addBlock', () => {
  ...
  it('can add model with props', () => {
    const doc = createTestDoc();
    doc.addBlock('blank:page', { title: new Text('hello') });
  ...
  it('can add multi models', () => {
    ...
    doc.addBlocks(
      [
        { flavour: 'blank:paragraph', blockProps: { type: 'h1' } },
        { flavour: 'blank:paragraph', blockProps: { type: 'h2' } },
      ],
      noteId
    );
    ...
    assert.deepEqual(serializCollection(doc.rootDoc).spaces[spaceId].blocks, {
      ...
      '1': {
        'sys:children': ['2', '3', '4'],
        'sys:flavour': 'blank:note',
        'sys:id': '1',
        'sys:version': 1,
      },
      '2': {
        'sys:children': [],
        'sys:flavour': 'blank:paragraph',
        'sys:id': '2',
        'prop:text': '',
        'prop:type': 'text',
        'sys:version': 1,
  ...
  it('can observe slot events', async () => {
    ...
    assert.equal(block.flavour, 'blank:page');
  });
  ...
  it('can add block to root', async () => {
    ...
    queueMicrotask(() => {
      const rootId = doc.addBlock('blank:page');
      noteId = doc.addBlock('blank:note', {}, rootId);
    });
    await waitOnce(doc.slots.rootAdded);
    const { root } = doc;
    if (!root) throw new Error('root is null');

    assert.equal(root.flavour, 'blank:page');

    doc.addBlock('blank:paragraph', {}, noteId!);
    assert.equal(root.children[0].flavour, 'blank:note');
    assert.equal(root.children[0].children[0].flavour, 'blank:paragraph');
...
describe('deleteBlock', () => {
  ...
  it('bring children to parent', () => {
    ...
    const rootId = doc.addBlock('blank:page', {});
    const noteId = doc.addBlock('blank:note', {}, rootId);
    const p1 = doc.addBlock('blank:paragraph', {}, noteId);
    doc.addBlock('blank:paragraph', {}, p1);
    doc.addBlock('blank:paragraph', {}, p1);
  ...
  it('bring children to other block', () => {
    ...
    const noteId = doc.addBlock('blank:note', {}, rootId);
    const p1 = doc.addBlock('blank:paragraph', {}, noteId);
    const p2 = doc.addBlock('blank:paragraph', {}, noteId);
    doc.addBlock('blank:paragraph', {}, p1);
    doc.addBlock('blank:paragraph', {}, p1);
    doc.addBlock('blank:paragraph', {}, p2);
  ...
  it('can delete model with parent', () => {
    ...
    const noteId = doc.addBlock('blank:note', {}, rootModel.id);
...
describe('getBlock', () => {
  it('can get block by id', () => {
    ...
    assert.equal(text.flavour, 'blank:paragraph');
