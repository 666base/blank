...
export class TableSelectionController implements ReactiveController {
  ...
  focusToCell(position: 'left' | 'right' | 'up' | 'down') {
    ...
    const rows = Array.from(
      row
        ?.closest('.blank-database-table-container')
    ...
    const container =
      groupKey != null
        ? this.tableContainer?.querySelector(
            `blank-data-view-table-group[data-group-key="${groupKey}"]`
          )
        : this.tableContainer;
  ...
  startDrag(
    ...
    const groupKey = cell.closest<TableGroup>('blank-data-view-table-group')
      ?.group?.key;
...
export class SelectionElement extends WithDisposable(ShadowlessElement) {
  ...
  updateFocusSelectionStyle(
    ...
    div.style.borderColor = 'var(--blank-primary-color)';
