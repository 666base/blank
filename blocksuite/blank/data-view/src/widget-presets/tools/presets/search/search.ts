...
const styles = css`
  .blank-database-search-container {
    position: relative;
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .blank-database-search-input-icon {
    position: absolute;
    left: 0;
    font-size: 20px;
    display: flex;
  ...
  .blank-database-search-input-icon:hover {
    background: var(--blank-hover-color);
  }

  .search-container-expand .blank-database-search-input-icon {
    left: 4px;
    pointer-events: none;
  }

  .blank-database-search-input {
    flex: 1;
    width: 100%;
  ...
  .blank-database-search-input::placeholder {
...
> {
  ...
  override render() {
    const searchToolClassMap = classMap({
      'blank-database-search-container': true,
      'search-container-expand': this.showSearch,
      active: this.showSearch,
    });
    return html`
      <label class="${searchToolClassMap}" @click="${this._clickSearch}">
        <div class="blank-database-search-input-icon">${SearchIcon()}</div>
        <input
          placeholder="Search..."
          class="blank-database-search-input"
  ...
  @query('.blank-database-search-input')
