import { Button, ErrorMessage, Skeleton, Tooltip } from '@blank/component';
    ...
    <div className={styles.storageProgressContainer}>
      <div className={styles.storageProgressWrapper}>
        <div className="storage-progress-desc">
          <span>{t['com.blank.storage.used.hint']()}</span>
          <span>
            {usedFormatted}/{maxFormatted}
            {` (${quotaName} ${t['com.blank.storage.plan']()})`}
          </span>
        </div>
      ...
      {hasPaymentFeature ? (
        <Tooltip
          ...
          content={
            isFreeUser
              ? t['com.blank.storage.maximum-tips']()
              : t['com.blank.storage.maximum-tips.pro']()
          }
        >
          <span tabIndex={0}>
            <Button variant={buttonType} onClick={onUpgrade}>
              {isFreeUser
                ? t['com.blank.storage.upgrade']()
                : t['com.blank.storage.change-plan']()}
            </Button>
          </span>
        </Tooltip>
      ) : null}
