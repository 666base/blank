import { clearMarksOnDiscontinuousInput } from '@blocksuite/blank-rich-text';
...
export const formatTextCommand: Command<{
  ...
  styles: BlankTextStyleAttributes;
  mode?: 'replace' | 'merge';
}> = (ctx, next) => {
              ...
              Object.entries(styles).map(([key, value]) => {
                if (typeof value === 'boolean') {
                  return [
                    key,
                    (inlineEditor.marks &&
                      inlineEditor.marks[key as keyof BlankTextAttributes]) ||
                    (delta &&
                      delta.attributes &&
                      delta.attributes[key as keyof BlankTextAttributes])
                      ? null
                      : value,
                  ];
                }
