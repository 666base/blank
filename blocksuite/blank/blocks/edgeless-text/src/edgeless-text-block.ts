import { TextUtils } from '@blocksuite/blank-block-surface';
import { formatBlockCommand } from '@blocksuite/blank-inline-preset';
import {
  DefaultTheme,
...
export class EdgelessTextBlockComponent extends GfxBlockComponent<EdgelessTextBlockModel> {
  static override styles = css`
    ...
    .edgeless-text-block-container blank-paragraph,
    blank-list {
  ...
  override renderGfxBlock() {
    ...
    const containerStyle: StyleInfo = {
      ...
      border: `1px solid ${editing ? 'var(--blank—primary—color, #1e96eb)' : 'transparent'}`,
      borderRadius: '4px',
  ...
  override renderPageContent() {
    ...
    const style = styleMap({
      ...
      '--blank-list-margin': '0',
      '--blank-paragraph-margin': '0',
    });

    return html`
      <div style=${style} class="blank-block-children-container">
  ...
  tryFocusEnd() {
    const paragraphOrLists = Array.from(
      this.querySelectorAll<BlockComponent>('blank-paragraph, blank-list')
    );
  ...
  @query('.blank-block-children-container')
  accessor childrenContainer!: HTMLDivElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'blank-edgeless-text': EdgelessTextBlockComponent;
  }
}
              ...
              view.updateComplete
                .then(() => {
                  ...
                  if (model.children.length === 0) {
                    ...
                    const rect = view
                      .querySelector('.blank-block-children-container')
