...
import { normalizeUrl } from '@blocksuite/blank-shared/utils';
...
export class BlankLink extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    blank-link a:hover [data-v-text='true'] {
  ...
  // see https://github.com/666base/blank/issues/1540
  ...
  override render() {
    ...
    if (this.delta.attributes && this.delta.attributes?.code) {
      const codeStyle = blankTextStyles(this.delta.attributes);
    ...
    const style = this.delta.attributes
      ? blankTextStyles(this.delta.attributes, linkStyle)
      : {};
