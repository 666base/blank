...
const styles = css`
  blank-data-view-table-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }

  blank-data-view-table-group {
    margin-top: 4px;
    padding-top: 4px;
    border-top: 1px solid var(--blank-border-color);
  }
  ...
  .blank-data-view-table-group:hover svg {
...
export class TableGroup extends SignalWatcher(
  ...
  private renderRows(rows: Row[]) {
    return html`
      ...
      ></blank-database-column-header>
      <div class="blank-database-block-rows">
        ${repeat(
          rows,
          row => row.rowId,
          (row, idx) => {
        ...
        : html` <div
          ...
          >
            <div
              ...
              data-test-id="blank-database-add-row-button"
              role="button"
            >
              ${PlusIcon()}<span style="font-size: 12px">New Record</span>
            </div>
          </div>`}
      <blank-database-column-stats
        .tableViewLogic="${this.tableViewLogic}"
        .group="${this.group}"
      >
      </blank-database-column-stats>
  ...
  @query('.blank-database-block-rows')
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-data-view-table-group': TableGroup;
  }
}
