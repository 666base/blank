  ...
  implements EditorSettingProvider
{
  ...
  constructor(
    ...
    const builtinServer =
      this.serversService.server$(DEFAULT_BUILTIN_SERVER_ID).value;
    if (!builtinServer) {
      throw new Error(`${DEFAULT_BUILTIN_SERVER_ID} server not found`);
    }
    const userDBService = builtinServer.scope.get(UserDBService);
