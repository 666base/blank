import type { ColumnDataType } from '@blocksuite/blank/model';
...
export function parseBlock(
  ...
  try {
    switch (flavour) {
      ...
      case 'blank:embed-linked-doc':
      case 'blank:embed-synced-doc': {
      ...
      case 'blank:database': {
        ...
        const optionToTagHtml = (option: any) => {
          return `<span data-blank-option data-value="${option.id}" data-option-color="${option.color}">${option.value}</span>`;
        };
  ...
  if (result.content && aiEditable && blockLevel === 2) {
    ...
    if (flavour === 'blank:database' || placeholder) {
...
export const parsePageDoc = (ctx: ParserContext): ParsedDoc => {
  ...
  if (!maybePageBlock) {
    ...
    rootBlock.children = rootBlock.children.filter(
      (block): block is BaseParsedBlock => block.flavour === 'blank:note'
    );
