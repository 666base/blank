...
import { FileSizeLimitProvider } from '@blocksuite/blank-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
export const AttachmentEmbedConfigIdentifier =
  createIdentifier<AttachmentEmbedConfig>(
    'BlankAttachmentEmbedConfigIdentifier'
  );
...
const embedConfig: AttachmentEmbedConfig[] = [
  {
    ...
    check: model =>
      model.store.schema.flavourSchemaMap.has('blank:image') &&
    ...
    render: (_, blobUrl) => {
      ...
      return html`
        <iframe
          ...
          credentialless
        ></iframe>
        <div class="blank-attachment-embed-event-mask"></div>
      `;
    },
  },
  {
...
async function turnIntoImageBlock(model: AttachmentBlockModel) {
  if (!model.store.schema.flavourSchemaMap.has('blank:image')) {
  ...
  transformModel(model, 'blank:image', imageProp);
}
