import { parseStringToRgba } from '@blocksuite/blank-components/color-picker';
import {
  ColorScheme,
  FrameBlockModel,
  isTransparent,
...
export const BLANK_FRAME_TITLE = 'blank-frame-title';

@requiredProperties({
  model: PropTypes.instanceOf(FrameBlockModel),
})
export class BlankFrameTitle extends SignalWatcher(
    ...
    _disposables.add(
      doc.slots.blockUpdated.subscribe(payload => {
        if (
          ...
          (payload.type === 'add' && payload.flavour === 'blank:frame')
        ) {
          this._nestedFrame = this._isInsideFrame();
        }
