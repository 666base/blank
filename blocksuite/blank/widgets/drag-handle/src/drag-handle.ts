...
import { EdgelessCRUDIdentifier } from '@blocksuite/blank-block-surface';
...
import type { BLANK_DRAG_HANDLE_WIDGET } from './consts.js';
...
export class BlankDragHandleWidget extends WidgetComponent<RootBlockModel> {
  ...
  override render() {
    ...
    const classes = {
      'blank-drag-handle-grabber': true,
      dots: isGfx ? true : false,
    };

    return html`
      <div class="blank-drag-handle-widget">
        <div class="blank-add-block-widget-container">
          <blank-add-block-widget
            .visible=${this.showAddBlockWidget && this.mode === 'page'}
            @add-block=${this._handleAddBlock}
          ></blank-add-block-widget>
        </div>
        <div class="blank-drag-handle-container">
        ...
        <div class="blank-drag-hover-rect" style=${hoverRectStyle}></div>
      </div>
    `;
  }

  @query('.blank-drag-handle-container')
  accessor dragHandleContainer!: HTMLDivElement;

  @query('.blank-drag-handle-grabber')
  accessor dragHandleGrabber!: HTMLDivElement;

  @query('.blank-add-block-widget-container')
...
declare global {
  interface HTMLElementTagNameMap {
    [BLANK_DRAG_HANDLE_WIDGET]: BlankDragHandleWidget;
  }
}
