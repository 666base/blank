...
import type { GroupElementModel } from '@blocksuite/blank-model';
...
export const group: ElementRenderer<GroupElementModel> = (
  ...
  if (elements.includes(model.id)) {
    if (model.showTitle) {
      ...
      ctx.strokeStyle = renderer.getPropertyValue('--blank-blue');
...
function renderTitle(
  ...
  ctx.font = font;
  ctx.fillStyle = renderer.getPropertyValue('--blank-blue');
  ctx.textAlign = 'left';
