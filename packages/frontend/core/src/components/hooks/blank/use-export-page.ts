...
import type { BlankEditorContainer } from '@blank/core/blocksuite/block-suite-editor/blocksuite-editor';
...
import { printToPdf } from '@blocksuite/blank/shared/utils';
import type { BlockStdScope } from '@blocksuite/blank/std';
import { type Store, Transformer } from '@blocksuite/blank/store';
import {
  createAssetsArchive,
  download,
  HtmlTransformer,
  ...
  const onClickHandler = useAsyncCallback(
    async (type: ExportType) => {
      ...
      const originEditorContainer = (editorContainer as any)
        .origin as BlankEditorContainer;
      ...
      try {
        ...
        if (!success) {
          notify.error({
            title: t['com.blank.export.error.title'](),
            message: t['com.blank.export.error.message'](),
          });
          return;
        }
        ...
        if (type === 'copy-markdown') {
          notify.success({
            title: t['com.blank.export.copied-as-markdown'](),
          });
        } else {
          notify.success({
            title: t['com.blank.export.success.title'](),
            message: t['com.blank.export.success.message'](),
          });
        }
      } catch (err) {
        console.error(err);
