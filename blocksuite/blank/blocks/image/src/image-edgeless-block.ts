...
import { cssVarV2, unsafeCSSVarV2 } from '@blocksuite/blank-shared/theme';
...
export class ImageEdgelessBlockComponent extends GfxBlockComponent<ImageBlockModel> {
  ...
  static override styles = css`
    blank-edgeless-image {
      position: relative;
    }

    blank-edgeless-image .loading {
      display: flex;
      align-items: center;
    ...
    blank-edgeless-image .blank-image-status {
      position: absolute;
      left: 18px;
      bottom: 18px;
    }
  ...
  override renderGfxBlock() {
    ...
    return html`
      <div class="blank-image-container" style=${containerStyleMap}>
        ${when(
          ...
          () => html`
            ...
            ${when(
              ...
              () =>
                html`<blank-resource-status
                  class="blank-image-status"
                  .message=${description}
        ...
        <blank-block-selection .block=${this}></blank-block-selection>
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-edgeless-image': ImageEdgelessBlockComponent;
  }
}
