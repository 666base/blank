...
> = (ctx, next) => {
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('blank:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('blank:paragraph', {}, parent);
    }
  ...
  const transformToLatex: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    blockModels.forEach(model => {
      ...
      const newId = transformModel(model, 'blank:latex', { latex });
      if (!newId) {
        return;
      }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain
        ...
        .pipe(selectBlocks),
      chain.pipe((_, next) => {
        if (['blank:code', 'blank:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
