...
export const BLANK_VIEWPORT_OVERLAY_WIDGET = 'blank-viewport-overlay-widget';

export class BlankViewportOverlayWidget extends WidgetComponent<RootBlockModel> {
  static override styles = css`
    .blank-viewport-overlay-widget {
      position: absolute;
      top: 0;
      left: 0;
      background: transparent;
      pointer-events: none;
      z-index: calc(var(--blank-z-index-popover) - 1);
    }

    .blank-viewport-overlay-widget.lock {
      pointer-events: auto;
    }
  `;
  ...
  override render() {
    const classes = classMap({
      'blank-viewport-overlay-widget': true,
...
export const viewportOverlayWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(BLANK_VIEWPORT_OVERLAY_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [BLANK_VIEWPORT_OVERLAY_WIDGET]: BlankViewportOverlayWidget;
  }
}
