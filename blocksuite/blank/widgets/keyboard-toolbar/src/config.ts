import { addSiblingAttachmentBlocks } from '@blocksuite/blank-block-attachment';
import { insertDatabaseBlockCommand } from '@blocksuite/blank-block-database';
import { insertEmptyEmbedIframeCommand } from '@blocksuite/blank-block-embed';
import { insertImagesCommand } from '@blocksuite/blank-block-image';
import { insertLatexBlockCommand } from '@blocksuite/blank-block-latex';
import {
  canDedentListCommand,
  canIndentListCommand,
  dedentListCommand,
  indentListCommand,
} from '@blocksuite/blank-block-list';
import { updateBlockType } from '@blocksuite/blank-block-note';
import {
  canDedentParagraphCommand,
  canIndentParagraphCommand,
  dedentParagraphCommand,
  indentParagraphCommand,
} from '@blocksuite/blank-block-paragraph';
import { DefaultTool, getSurfaceBlock } from '@blocksuite/blank-block-surface';
import { insertSurfaceRefBlockCommand } from '@blocksuite/blank-block-surface-ref';
import { insertTableBlockCommand } from '@blocksuite/blank-block-table';
...
import { insertInlineLatex } from '@blocksuite/blank-inline-latex';
import { toggleLink } from '@blocksuite/blank-inline-link';
import {
  formatBlockCommand,
  formatNativeCommand,
  formatTextCommand,
...
import type { BlankLinkedDocWidget } from '@blocksuite/blank-widget-linked-doc';
...
const textToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    icon: TextIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('blank:paragraph'),
    action: ({ std }) => {
  ...
  {
    name: 'CodeBlock',
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('blank:code'),
    icon: CodeBlockIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'blank:code',
  ...
  },
  {
    name: 'Equation',
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('blank:latex'),
    icon: TeXIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'blank:latex',
  ...
  },
  {
    ...
    icon: DividerIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('blank:divider'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  },
  {
    ...
    showWhen: ({ std, rootComponent: { model } }) =>
      std.store.schema.flavourSchemaMap.has('blank:table') &&
      !isInsideBlockByFlavour(std.store, model, 'blank:edgeless-text'),
    action: ({ std }) => {
      std.command
        .chain()
  ...
  },
  {
    ...
    action: ({ rootComponent: { model }, std }) => {
      ...
      if (index === -1) return;
      const calloutId = store.addBlock('blank:callout', {}, parent, index + 1);
      if (!calloutId) return;
      const paragraphId = store.addBlock('blank:paragraph', {}, calloutId);
...
const listToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('blank:list'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'blank:list',
        props: {
  ...
  items: [
    {
      ...
      icon: NewPageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:embed-linked-doc'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      showWhen: ({ std, rootComponent }) => {
        const linkedDocWidget = std.view.getWidget(
          'blank-linked-doc-widget',
          rootComponent.model.id
        );
        if (!linkedDocWidget) return false;

        return std.store.schema.flavourSchemaMap.has('blank:embed-linked-doc');
      },
      action: ({ rootComponent, closeToolPanel }) => {
        ...
        assertType<BlankLinkedDocWidget>(linkedDocWidget);
  ...
  items: [
    {
      ...
      icon: ImageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:image'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      icon: LinkIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:bookmark'),
      action: async ({ std }) => {
  ...
  items: [
    {
      ...
      showWhen: ({ std }) => {
        return std.store.schema.flavourSchemaMap.has('blank:embed-iframe');
      },
      action: async ({ std }) => {
        std.command
          .chain()
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:embed-youtube'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:embed-github'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:embed-figma'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:embed-loom'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
...
const documentGroupFrameToolGroup: DynamicKeyboardToolPanelGroup = ({
  ...
  const frameModels = store
    .getBlocksByFlavour('blank:frame')
  ...
  items: [
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('blank:database'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
        ...
        ...(
          ...
          action: ({ std }) => {
            const payload = {
              ...
              } satisfies BlankTextStyleAttributes,
            };
            std.command
              .chain()
              .try(chain => [
...
export const defaultKeyboardToolbarConfig: KeyboardToolbarConfig = {
  items: [
    moreToolPanel,
    // TODO(@L-Sun): add ai function in Blank side
