...
export default {
  ...
  async viteFinal(config, _options) {
    const { getBuildConfig } = await import('@blank-tools/utils/build-config');
    const { Package } = await import('@blank-tools/utils/workspace');
    ...
    return mergeConfig(config, {
      ...
      define: Object.entries(
        getBuildConfig(new Package('@blank/web'), {
          mode: 'development',
          channel: 'canary',
        })
