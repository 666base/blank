import { useDocMetaHelper } from '@blank/core/components/hooks/use-block-suite-page-meta';
import { useDocCollectionPage } from '@blank/core/components/hooks/use-block-suite-workspace-page';
...
import type { ListHistoryQuery } from '@blank/graphql';
import { listHistoryQuery, recoverDocMutation } from '@blank/graphql';
...
export const useSnapshotPage = (
  ...
  const blankWorkspace = useService(WorkspaceService).workspace;
  ...
  const page = useMemo(() => {
    ...
    const historyShellWorkspace = getOrCreateShellWorkspace(
      docCollection.id,
      workspacesService.getWorkspaceFlavourProvider(blankWorkspace.meta)
    );
  ...
  }, [blankWorkspace.meta, docCollection, graphQLService, workspacesService]);
