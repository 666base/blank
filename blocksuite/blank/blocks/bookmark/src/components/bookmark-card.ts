import { getEmbedCardIcons } from '@blocksuite/blank-block-embed';
import { LoadingIcon, WebIcon16 } from '@blocksuite/blank-components/icons';
import { ImageProxyService } from '@blocksuite/blank-shared/adapters';
import { ThemeProvider } from '@blocksuite/blank-shared/services';
import { getHostName } from '@blocksuite/blank-shared/utils';
...
export class BookmarkCard extends SignalWatcher(
  ...
  override render() {
    ...
    return html`
      <div
        class="blank-bookmark-card ${cardClassMap}"
        @click=${this.bookmark.handleClick}
        @dblclick=${this.bookmark.handleDoubleClick}
      >
        <div class="blank-bookmark-content">
          <div class="blank-bookmark-content-title">
            <div class="blank-bookmark-content-title-icon">${titleIcon}</div>
            <div class="blank-bookmark-content-title-text">${titleText}</div>
          </div>
          <div class="blank-bookmark-content-description">
            ${descriptionText}
          </div>
          <div class="blank-bookmark-content-url-wrapper">
            <div
              class="blank-bookmark-content-url"
              @click=${this.bookmark.open}
            >
              <span>${getHostName(url)}</span>
              <div class="blank-bookmark-content-url-icon">
                ${OpenInNewIcon({ width: '12', height: '12' })}
              </div>
            </div>
          </div>
        </div>
        <div class="blank-bookmark-banner">${bannerImage}</div>
      </div>
    `;
  }
