...
import type { BlankFrameTitleWidget } from '@blocksuite/blank/widgets/frame-title';
  ...
  test('frame should have title', async () => {
    ...
    const getFrameTitle = (frameId: string) => {
      const frameTitleWidget = service.std.view.getWidget(
        'blank-frame-title-widget',
        frameId
      ) as BlankFrameTitleWidget | null;
      return frameTitleWidget?.shadowRoot?.querySelector('blank-frame-title');
    };
