import { getScrollContainer } from '@blocksuite/blank-shared/utils';
...
export class DatabaseColumnHeader extends SignalWatcher(
  ...
  editLastColumnTitle = () => {
    const columns = this.querySelectorAll('blank-database-header-column');
  ...
  override connectedCallback() {
    ...
    const scrollContainer = getScrollContainer(
      this.closest('blank-data-view-renderer')!
    );
    const group = this.closest('blank-data-view-table-group');
    if (group) {
  ...
  override render() {
    return html`
      ...
      <div class="blank-database-column-header database-row">
        ${this.readonly
          ? nothing
          ...
          (column, index) => {
            ...
            return html`
              <blank-database-header-column
                ...
                class="blank-database-column database-cell"
                .column="${column}"
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-database-column-header': DatabaseColumnHeader;
  }
}
