import { Avatar, Skeleton, Tooltip } from '@blank/component';
...
export const MembersRow = ({ onClick }: { onClick: () => void }) => {
  ...
  const description = useMemo(() => {
    ...
    switch (grantedUserCount) {
      case 2:
        return t['com.blank.share-menu.member-management.member-count-2']({
          member1: topThreeMembers[0].name,
          member2: topThreeMembers[1].name,
        });
      case 3:
        return t['com.blank.share-menu.member-management.member-count-3']({
          member1: topThreeMembers[0].name,
          member2: topThreeMembers[1].name,
          member3: topThreeMembers[2].name,
        });
      default:
        return t['com.blank.share-menu.member-management.member-count-more']({
          member1: topThreeMembers[0].name,
          member2: topThreeMembers[1].name,
  ...
  return (
    ...
    >
      {docOwner ? (
        ...
        <div>{t['com.blank.share-menu.invite-editor.manage-members']()}</div>
      )}
      <div className={styles.IconButtonStyle}>
        <ArrowRightSmallIcon />
      </div>
