...
function toggleStyle(
  inlineEditor: BlankInlineEditor | null,
  attrs: BlankTextAttributes
): void {
  ...
  let oldAttributes: BlankTextAttributes = {};
...
export class RichTextCell extends BaseCellRenderer<Text, string> {
  ...
  get topContenteditableElement() {
    const databaseBlock =
      this.closest<DatabaseBlockComponent>('blank-database');
  ...
  insertDelta = (delta: DeltaInsert<BlankTextAttributes>) => {
    const inlineEditor = this.inlineEditor$.value;
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-database-rich-text-cell': RichTextCell;
  }
}
