...
function AppContent() {
  ...
  const [mobileTab, setMobileTab] = useState<"notes" | "calendar" | "editor">("notes");
  const editorRef = useRef<TiptapEditor | null>(null);

  // Update mobile tab when a note is selected or focus changes
  useEffect(() => {
    if (selectedNoteId) {
      setMobileTab("editor");
    } else {
  ...
  const handleCalendarSelect = useCallback(() => {
    ...
    const existing = notes.find((n) => n.title.includes(today));
    if (existing) {
      selectNote(existing.id);
    } else {
      createNote();
      // Future: Could pre-fill with today's date if API supported it
    }
  }, [notes, selectNote, createNote]);

  // Listen for set-notes-folder event from CLI (blank .)
      ...
      <div className="h-full min-h-0 flex bg-bg text-text overflow-hidden">
        {view === "settings" ? (
          ...
          <>
            <div
              data-sidebar
              className={`transition-all duration-500 ease-out overflow-hidden md:flex ${!sidebarVisible || focusMode ? "opacity-0 -translate-x-4 w-0 pointer-events-none md:w-0" : "opacity-100 translate-x-0 w-full md:w-64"} ${mobileTab !== "notes" ? "hidden md:block" : ""}`}
            >
              <Sidebar onOpenSettings={toggleSettings} />
            </div>
            <div className={`flex-1 flex-col min-w-0 flex ${mobileTab !== "editor" && mobileTab !== "calendar" ? "hidden md:flex" : ""}`}>
              <Editor
            ...
            <div
              data-right-sidebar
              className={`transition-all duration-500 ease-out overflow-hidden hidden md:flex ${!rightSidebarVisible || focusMode ? "opacity-0 translate-x-4 w-0 pointer-events-none md:w-0" : "opacity-100 translate-x-0 md:w-64"}`}
            >
              <CalendarSidebar />
            </div>
          </>
...
function UpdateToast({
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast.success("Update installed! Restart Blank to apply.", {
        duration: Infinity,
        closeButton: true,
      });
    } catch (err) {
