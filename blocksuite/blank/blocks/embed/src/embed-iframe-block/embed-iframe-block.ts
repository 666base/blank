...
export class EmbedIframeBlockComponent extends CaptionedBlockComponent<EmbedIframeBlockModel> {
  ...
  private readonly _renderIframe = () => {
    ...
    return html`<iframe
      ...
      ></iframe>
      ${sourceHost
        ? html`<div class="blank-embed-iframe-source">${sourceHost}</div>`
        : nothing}`;
  };
  ...
  override renderBlock() {
    const containerClasses = classMap({
      'blank-embed-iframe-block-container': true,
    ...
    const overlayClasses = classMap({
      'blank-embed-iframe-block-overlay': true,
  ...
  @query('.blank-embed-iframe-block-container')
