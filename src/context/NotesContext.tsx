...
export function NotesProvider({ children }: { children: ReactNode }) {
  ...
  const saveNote = useCallback(
    async (content: string, noteId?: string) => {
      ...
      try {
        ...
        // Push to cloud sync asynchronously
        if (updated) {
          syncService.pushNote(updated).catch(err => {
  ...
  const deleteNote = useCallback(
    async (id: string) => {
      try {
        ...
        syncService.deleteNote(id).catch(err => {
  ...
  const duplicateNote = useCallback(
    async (id: string) => {
      try {
        ...
        // Push duplicated note to cloud sync
        syncService.pushNote(newNote).catch(err => {
          console.error('Failed to sync duplicated note to cloud:', err);
        });
    ...
    async function init() {
      try {
        ...
        if (folder) {
          ...
          // Pull notes from cloud on startup if authenticated
          syncService.pullNotes().then(() => refreshNotes()).catch(console.error);
        }
      } catch (err) {
  ...
  // Listen to Supabase auth changes to trigger pull
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, _session) => {
      if (event === 'SIGNED_IN' && notesFolder) {
        syncService.pullNotes().then(() => refreshNotes()).catch(console.error);
      }
    });
