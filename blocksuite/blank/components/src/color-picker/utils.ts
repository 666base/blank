...
import type { Color, ColorScheme } from '@blocksuite/blank-model';
...
export const parseStringToRgba = (value: string) => {
  ...
  // Compatible old format: `--blank-palette-transparent`
 ...
 *
 * ```json
 * { 'fillColor': '--blank-palette-shape-yellow' }
 * { 'fillColor': '#ffffff' }
...
export const calcCustomButtonStyle = (
  ...
  if (color.startsWith('--')) {
    if (!color.endsWith('transparent')) {
      b = 'var(--blank-background-overlay-panel-color)';
      c = keepColor(
        rgbaToHex8(
