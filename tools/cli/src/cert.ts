import { type Path, ProjectRoot } from '@blank-tools/utils/path';
...
const CA_PEM_PATH = CA_DIR.join('blank-self-signed.pem');
const CA_KEY_PATH = CA_DIR.join('blank-self-signed.key');

const CA_ORG = 'Blank Dev CA Self Signed Org';
const CA_NAME = 'Blank Dev CA Self Signed CN';
...
export class CertCommand extends Command {
  ...
  domain = Option.String('--domain', {
    description:
      'Generate certificates for given domain. e.g. "blank.localhost"',
  });
  ...
  private createCert(domain: string) {
    if (!this.checkInstalled(CA_PEM_PATH)) {
      this.logger.error(
        'CA not installed. Please run `yarn blank cert --install` first.'
      );
      process.exit(1);
    }
    ...
    this.exec(
      `openssl req -new -key ${keyPath} -out ${csrPath} -config ${confPath} -subj "/C=/ST=/O=/localityName=/commonName=${domain}/organizationalUnitName=/emailAddress=${domain}@blank.pro/"`
    );
    this.exec(
  ...
  private installCa() {
    ...
    this.exec(
      `openssl req -new -newkey rsa:2048 -days 1024 -nodes -x509 -subj "/C=/ST=/O=${CA_ORG}/localityName=/commonName=${CA_NAME}/organizationalUnitName=Developers/emailAddress=dev@blank.pro/" -keyout ${CA_KEY_PATH} -out ${CA_PEM_PATH}`
    );
    this.trustCa(CA_PEM_PATH);
  }

  private trustCa(pem: Path) {
    this.logger.info(`Trusting Blank Dev Self Signed CA`);
    this.exec(
