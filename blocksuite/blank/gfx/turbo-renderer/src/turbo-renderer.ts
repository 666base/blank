 ...
 *
 * To add Turbo Rendering support for a new block type (e.g., 'blank:my-block'):
 *
 * 1.  **In the block's package (e.g., `blocksuite/blank/blocks/my-block`):**
 *   a.  Add `@blocksuite/blank/gfx/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
 *   b.  Implement the Layout Handler (e.g., `MyBlockLayoutHandlerExtension`) and Painter Worker (e.g., `MyBlockLayoutPainterExtension`). Refer to `ParagraphLayoutHandlerExtension` and `ParagraphLayoutPainterExtension` in `blocksuite/blank/blocks/block-paragraph` for implementation examples.
 ...
 *   e.  Add a TypeScript project reference to `blocksuite/blank/gfx/turbo-renderer` in `tsconfig.json`.
 *
 ...
 *
 * 3.  **Run `yarn blank init`** from the workspace root to update generated configuration files (`workspace.gen.ts`) and the lockfile (`yarn.lock`).
 *
 * **Note:** Always ensure the directory structure and export patterns match the `paragraph` block (`blocksuite/blank/blocks/block-paragraph`) for consistency.
 */
export class ViewportTurboRendererExtension extends GfxExtension {
  ...
  override mounted() {
    const mountPoint = document.querySelector('.blank-edgeless-viewport');
    if (mountPoint) {
