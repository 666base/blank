...
import { DocListItemComponent } from '@blank/core/components/explorer/docs-view/docs-list';
import { Filters } from '@blank/core/components/filter';
import { BlankShapeIcon } from '@blank/core/components/page-list';
import type { CollectionInfo } from '@blank/core/modules/collection';
...
export const RulesMode = ({
        ...
        },
        {
          ...
          children: (
            <div className={styles.includeListTitle}>
              {t['com.blank.editCollection.rules.include.title']()}
            </div>
          ),
  ...
  const tips = useMemo(
    () => (
      <Trans
        i18nKey="com.blank.editCollection.rules.tips"
        values={{
          highlight: t['com.blank.editCollection.rules.tips.highlight'](),
        }}
        components={{
    ...
    <>
                      ...
                      <div className={styles.includeItem} key={id}>
                        <div className={styles.includeItemContent}>
                          ...
                          >
                            ...
                            {t[
                              'com.blank.editCollection.rules.include.page'
                            ]()}
                          </div>
                          <div className={styles.includeItemContentIs}>
                            {t['com.blank.editCollection.rules.include.is']()}
                          </div>
                          <DocTitle id={id} />
                        </div>
                        <IconButton
      ...
      <div className={styles.rulesBottom}>
        <div className={styles.bottomLeft}>
          ...
          >
            {t['com.blank.editCollection.rules.preview']()}
          </Button>
          <Button variant="plain" onClick={reset}>
            {t['com.blank.editCollection.rules.reset']()}
          </Button>
          <div className={styles.previewCountTips}>
            <Trans
              i18nKey="com.blank.editCollection.rules.countTips"
              values={{
  ...
  return (
    ...
    >
      ...
      <strong style={{ fontSize: 20, lineHeight: '28px' }}>
        {noRules
          ? t['com.blank.editCollection.rules.empty.noRules']()
          : t['com.blank.editCollection.rules.empty.noResults']()}
      </strong>
      <div
        style={{
          width: '389px',
      ...
      >
        {noRules ? (
          <Trans i18nKey="com.blank.editCollection.rules.empty.noRules.tips">
            Please <strong>add rules</strong> to save this collection or switch
            to <strong>Pages</strong>, use manual selection mode
          </Trans>
        ) : (
          t['com.blank.editCollection.rules.empty.noResults.tips']()
        )}
      </div>
    </div>
  );
