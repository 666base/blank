...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [BlankInlineEditor, InlineRange | null][]
): BlankTextAttributes {
  const formatArr: BlankTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: BlankTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof BlankTextAttributes;
...
function getSelectedInlineEditors(
  ...
  filter: (
    inlineRoot: InlineRootElement<BlankTextAttributes>
  ) => InlineEditor<BlankTextAttributes> | []
) {
  return blocks.flatMap(el => {
    const inlineRoot = el.querySelector<
      InlineRootElement<BlankTextAttributes>
    >(`[${INLINE_ROOT_ATTR}]`);
...
function handleCurrentSelection(
  ...
  handler: (
    ...
    inlineEditors: InlineEditor<BlankTextAttributes>[]
  ) => { textAttributes: BlankTextAttributes } | boolean | void
): Chain<InitCommandCtx & { textAttributes: BlankTextAttributes }> {
  return chain.try(chain => [
    ...
    chain.pipe((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): BlankInlineEditor => el.inlineEditor);
