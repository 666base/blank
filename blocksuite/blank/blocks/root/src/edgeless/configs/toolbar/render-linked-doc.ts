import { isFrameBlock } from '@blocksuite/blank-block-frame';
import { getSurfaceBlock, isNoteBlock } from '@blocksuite/blank-block-surface';
import {
...
export function createLinkedDocFromNote(
  ...
  linkedDoc.load(() => {
    const rootId = linkedDoc.addBlock('blank:page', {
      title: new Text(docTitle),
    });
    linkedDoc.addBlock('blank:surface', {}, rootId);
    transformer
...
export function createLinkedDocFromEdgelessElements(
  ...
  linkedDoc.load(() => {
    ...
    const surfaceId = linkedDoc.addBlock('blank:surface', {}, rootId);
