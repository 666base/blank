...
import { AudioPlayer } from '@blank/component/ui/audio-player';
...
import { useSeekTime } from '@blank/core/components/hooks/use-seek-time';
import { CurrentServerScopeProvider } from '@blank/core/components/providers/current-server-scope';
import { PublicUserLabel } from '@blank/core/modules/cloud/views/public-user';
import { GlobalDialogService } from '@blank/core/modules/dialogs';
import type { AudioAttachmentBlock } from '@blank/core/modules/media/entities/audio-attachment-block';
import { AudioAttachmentService } from '@blank/core/modules/media/services/audio-attachment';
...
const AttachmentAudioPlayer = ({ block }: { block: AudioAttachmentBlock }) => {
  ...
  const handleNotesClick = useAsyncCallback(async () => {
    ...
    if (result?.error === 'created-by-others') {
      confirmModal.openConfirmModal({
        title: t['com.blank.audio.transcribe.non-owner.confirm.title'](),
        description: (
          <Trans i18nKey="com.blank.audio.transcribe.non-owner.confirm.message">
  ...
  const notesEntry = useMemo(() => {
    ...
    const inner = (
      ...
      >
        {transcribing
          ? t['com.blank.audio.transcribing']()
          : t['com.blank.audio.notes']()}
      </Button>
    );
    if (transcribing) {
      return (
        <Tooltip content={t['com.blank.audio.transcribing']()}>
          {inner}
        </Tooltip>
      );
    }
