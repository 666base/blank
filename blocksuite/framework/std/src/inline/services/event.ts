...
export class EventService<TextAttributes extends BaseTextAttributes> {
  ...
  private readonly _onBeforeInput = async (event: InputEvent) => {
    ...
    const rangeFromSelection = inlineRange ? { ...inlineRange } : null;
    ...
    if (ifHandleTargetRange) {
      ...
      if (targetRanges.length > 0) {
        ...
        // Mobile browsers (especially iOS Safari) may report a full-line
        // target range on Space/Enter while the visible caret is collapsed.
        // Trusting it would replace the whole line with a single character.
        const ignoreWrongTargetRange =
          IS_MOBILE &&
          rangeFromSelection?.length === 0 &&
          targetInlineRange &&
          targetInlineRange.length > 0 &&
