...
export class RichText extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _inlineEditor$ = signal<BlankInlineEditor | null>(null);
  ...
  private _init() {
    ...
    // init inline editor
    this._inlineEditor$.value = new InlineEditor<BlankTextAttributes>(
      this._yText,
      {
  ...
  accessor embedChecker: <
    TextAttributes extends BlankTextAttributes = BlankTextAttributes,
  >(
  ...
  @property({ attribute: false })
  accessor markdownMatches: InlineMarkdownMatch<BlankTextAttributes>[] = [];
