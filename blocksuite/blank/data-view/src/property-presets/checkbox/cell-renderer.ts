...
const playCheckAnimation = async (
  ...
  sparkingEl.classList.add('blank-check-animation');
  if (size < 20) {
...
export class CheckboxCell extends BaseCellRenderer<boolean> {
  static override styles = css`
    blank-database-checkbox-cell {
      display: block;
      width: 100%;
      cursor: pointer;
    }

    .blank-database-checkbox-container {
      height: 100%;
    }

    .blank-database-checkbox {
      display: flex;
      align-items: center;
  ...
  override render() {
    ...
    const icon = checked
      ...
      : CheckBoxUnIcon();
    return html` <div class="blank-database-checkbox-container">
      <div
        class="blank-database-checkbox checkbox ${checked ? 'checked' : ''}"
      >
        ${icon}
      </div>
    </div>`;
