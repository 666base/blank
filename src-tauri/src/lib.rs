...
// Per-folder settings (stored in .blank/settings.json within notes folder)
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct Settings {
...
pub struct AppState {
    ...
    pub settings: RwLock<Settings>,      // per-folder settings (stored in .blank/)
...
const EXCLUDED_DIRS: &[&str] = &[".git", ".blank", ".obsidian", ".trash", "assets"];
...
// Get per-folder settings file path (in .blank/ within notes folder)
fn get_settings_path(notes_folder: &str) -> PathBuf {
    let blank_dir = PathBuf::from(notes_folder).join(".blank");
    std::fs::create_dir_all(&blank_dir).ok();
    blank_dir.join("settings.json")
}
...
fn initialize_notes_folder(app: &AppHandle, path_buf: &PathBuf, state: &AppState) -> Result<String, String> {
    ...
    // Create .blank config folder
    let blank_dir = path_buf.join(".blank");
    std::fs::create_dir_all(&blank_dir).map_err(|e| e.to_string())?;

    // Verify write access early to avoid later silent failures
    let write_test_path = blank_dir.join(".write-test");
...
const RESERVED_FOLDER_NAMES: &[&str] = &[".git", ".blank", ".obsidian", ".trash", "assets"];
...
/// Marker comment embedded in CLI wrapper scripts installed by Blank.
/// Used to identify and validate our own wrapper before modifying or removing it.
#[cfg(target_os = "macos")]
const BLANK_CLI_MARKER: &str = "# BLANK_CLI_WRAPPER";
...
/// Apple Silicon: /opt/homebrew/bin/blank
/// Intel: /usr/local/bin/blank
#[cfg(target_os = "macos")]
fn cli_target_path() -> PathBuf {
    ...
    if let Ok(path_var) = std::env::var("PATH") {
        if path_var.split(':').any(|p| p == "/opt/homebrew/bin") {
            return PathBuf::from("/opt/homebrew/bin/blank");
        }
    }
    ...
    PathBuf::from("/usr/local/bin/blank")
}
...
fn get_cli_status() -> Result<CliStatus, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if !content.contains(BLANK_CLI_MARKER) {
...
fn install_cli() -> Result<String, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if target.exists() || target.symlink_metadata().is_ok() {
            ...
            if !content.contains(BLANK_CLI_MARKER) {
                return Err(format!(
                    "A different 'blank' command already exists at {}. Remove it manually to install the Blank CLI.",
                    target.display()
                ));
            }
...
fn uninstall_cli() -> Result<(), String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if target.exists() || target.symlink_metadata().is_ok() {
            ...
            if !content.contains(BLANK_CLI_MARKER) {
                return Err(format!(
                    "File at {} was not installed by Blank. Refusing to remove.",
                    target.display()
                ));
            }
            std::fs::remove_file(&target)
