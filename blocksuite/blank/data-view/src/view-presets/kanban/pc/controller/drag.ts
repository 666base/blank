import type { InsertToPosition } from '@blocksuite/blank-shared/utils';
...
export class KanbanDragController implements ReactiveController {
  dragStart = (ele: KanbanCard, evt: PointerEvent) => {
    ...
    const currentGroup = ele.closest('blank-data-view-kanban-group');
      ...
      this.host.disposables.add(
        this.logic.handleEvent('dragStart', context => {
          ...
          if (target instanceof Element) {
            const cell = target.closest('blank-data-view-kanban-cell');
            if (cell?.isEditing$.value) {
              return;
            }
            cell?.selectCurrentCell(false);
            const card = target.closest('blank-data-view-kanban-card');
            if (card) {
...
const createDragPreview = (card: KanbanCard, x: number, y: number) => {
  ...
  kanbanCard.style.backgroundColor = 'var(--blank-background-primary-color)';
...
const createDropPreview = () => {
  ...
  div.style.backgroundColor = 'var(--blank-primary-color)';
