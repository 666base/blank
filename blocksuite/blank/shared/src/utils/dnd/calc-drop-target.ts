import { DatabaseBlockModel, ListBlockModel } from '@blocksuite/blank-model';
...
export function calcDropTarget(
  ...
  const schema = model.store.schema.get('blank:database');
  ...
  if (!shouldAppendToDatabase && !matchModels(model, [DatabaseBlockModel])) {
    const databaseBlockComponent =
      element.closest<BlockComponent>('blank-database');
