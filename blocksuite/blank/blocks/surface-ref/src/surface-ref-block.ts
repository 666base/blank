import { FrameBlockComponent } from '@blocksuite/blank-block-frame';
import {
  EdgelessCRUDIdentifier,
  getSurfaceBlock,
} from '@blocksuite/blank-block-surface';
import type { BlockCaptionEditor } from '@blocksuite/blank-components/caption';
...
export class SurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    ...
    blank-surface-ref:not(:hover)
      blank-surface-ref-toolbar:not([data-open-menu-display='show']) {
      display: none;
    }
    ...
    .blank-surface-ref.focused {
      border-color: ${unsafeCSSVarV2('edgeless/frame/border/active')};
    }

    .blank-surface-ref.comment-highlighted {
  ...
  private get _shouldRender() {
    return (
      ...
      !this.parentComponent?.closest('blank-surface-ref')
    );
  }
  ...
  @query('.blank-surface-ref')
  accessor hoverableContainer!: HTMLDivElement;

  @query('blank-surface-ref > block-caption-editor')
...
declare global {
  interface HTMLElementTagNameMap {
    'blank-surface-ref': SurfaceRefBlockComponent;
  }
}
