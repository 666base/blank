...
import { ExternalLinksQuickSearchSession } from '@blank/core/modules/quicksearch/impls/external-links';
import { JournalsQuickSearchSession } from '@blank/core/modules/quicksearch/impls/journals';
import { track } from '@blank/track';
import {
  BookmarkSlashMenuConfigIdentifier,
  insertLinkByQuickSearchCommand,
} from '@blocksuite/blank/blocks/bookmark';
import { LinkedDocSlashMenuConfigIdentifier } from '@blocksuite/blank/blocks/embed-doc';
import type { ServiceIdentifier } from '@blocksuite/blank/global/di';
import {
...
import type {
  SlashMenuConfig,
  SlashMenuItem,
} from '@blocksuite/blank/widgets/slash-menu';
    ...
    async openQuickSearch() {
      ...
      searchResult = await new Promise((resolve, reject) =>
        framework.get(QuickSearchService).quickSearch.show(
          ...
          {
            label: {
              i18nKey: 'com.blank.cmdk.insert-links',
            },
            placeholder: {
          ...
          item.action = ({ std }) => {
            ...
            insertedLinkType
              ?.then(type => {
                ...
                if (flavour === 'blank:bookmark') {
                  track.doc.editor.slashMenu.bookmark();
                  return;
                }

                if (flavour === 'blank:embed-linked-doc') {
