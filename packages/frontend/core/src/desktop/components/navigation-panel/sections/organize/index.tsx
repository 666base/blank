...
export const NavigationPanelOrganize = () => {
  ...
  const handleOnChildrenDrop = useCallback(
    (data: DropTargetDropEvent<BlankDNDData>, node?: FolderNode) => {
      if (!node || !node.id) {
      ...
      if (
        ...
        if (data.source.data.entity?.type === 'folder') {
          ...
          toast(t['com.blank.rootAppSidebar.organize.root-folder-only']());
        }
      } else {
    ...
    <CollapsibleSection
      ...
      actions={
        <IconButton
          ...
          size="16"
          tooltip={t[
            'com.blank.rootAppSidebar.explorer.organize-section-add-tooltip'
          ]()}
        >
          <AddOrganizeIcon />
        </IconButton>
