...
export const NoteSettings = () => {
  ...
  const borderStyleItems = useMemo<RadioItem[]>(
    () => [
      ...
      },
      {
        ...
        label:
          t['com.blank.settings.editorSettings.edgeless.note.border.none'](),
      },
    ],
    [t]
  );

  const { borderStyle } = settings['blank:note'].edgeless.style;
  const setBorderStyle = useCallback(
    (value: StrokeStyle) => {
      editorSetting.set('blank:note', {
        edgeless: {
          style: {
            borderStyle: value,
          },
  ...
  const { borderSize } = settings['blank:note'].edgeless.style;
  ...
  const backgroundItems = useMemo(() => {
    const { background } = settings['blank:note'];
    return palettes.map(({ key, value, resolvedValue }) => {
      const handler = () => {
        editorSetting.set('blank:note', { background: value });
      };
  ...
  const cornerItems = useMemo(() => {
    const { borderRadius } = settings['blank:note'].edgeless.style;
  ...
  const shadowItems = useMemo(() => {
    const { shadowType } = settings['blank:note'].edgeless.style;
  ...
  const getElements = useCallback((doc: Store) => {
    return doc.getBlocksByFlavour('blank:note') || [];
  }, []);

  return (
    <>
      <EdgelessSnapshot
        title={t['com.blank.settings.editorSettings.edgeless.note']()}
        docName="note"
        keyName="blank:note"
      ...
      <SettingRow
        name={t[
          'com.blank.settings.editorSettings.edgeless.note.background'
        ]()}
        desc={''}
      >
        {currentColor ? (
      ...
      <SettingRow
        name={t['com.blank.settings.editorSettings.edgeless.note.corners']()}
        desc={''}
      >
        <DropdownMenu
          items={cornerItems}
          trigger={
            <MenuTrigger className={menuTrigger}>
              {
                CornerSizeMap[
                  settings['blank:note'].edgeless.style
      ...
      <SettingRow
        name={t['com.blank.settings.editorSettings.edgeless.note.shadow']()}
        desc={''}
      >
        <DropdownMenu
          items={shadowItems}
          trigger={
            <MenuTrigger className={menuTrigger}>
              {NoteShadowMap[settings['blank:note'].edgeless.style.shadowType]}
            </MenuTrigger>
          }
        />
      </SettingRow>
      <SettingRow
        name={t['com.blank.settings.editorSettings.edgeless.note.border']()}
        desc={''}
      >
        <RadioGroup
      ...
      <SettingRow
        name={t[
          'com.blank.settings.editorSettings.edgeless.note.border-thickness'
        ]()}
        desc={''}
      >
        <Slider
