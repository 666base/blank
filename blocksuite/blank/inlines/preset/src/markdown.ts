import { LatexExtension } from '@blocksuite/blank-inline-latex';
...
export const BoldItalicMarkdown = InlineMarkdownExtension<BlankTextAttributes>(
  {
    name: 'bolditalic',
...
export const BoldMarkdown = InlineMarkdownExtension<BlankTextAttributes>({
  name: 'bold',
...
export const ItalicExtension = InlineMarkdownExtension<BlankTextAttributes>({
  name: 'italic',
...
export const StrikethroughExtension =
  InlineMarkdownExtension<BlankTextAttributes>({
    name: 'strikethrough',
    pattern: /.*~{2}([^\s][^~]*[^\s])~{2}\s$|.*~{2}([^\s~])~{2}\s$/,
    action: ({
      inlineEditor,
...
export const CodeExtension = InlineMarkdownExtension<BlankTextAttributes>({
  name: 'code',
  pattern: /.*`([^\s][^`]*[^\s])`\s$|.*`([^\s`])`\s$/,
  action: ({ inlineEditor, prefixText, inlineRange, pattern, undoManager }) => {
    const match = prefixText.match(pattern);
