...
import { createDefaultDoc, matchModels } from '@blocksuite/blank-shared/utils';
  ...
  implements DetailSlotProps
{
  ...
  protected override render(): unknown {
    return html`
      <div
        style="height: 1px;max-width: var(--blank-editor-width);background-color: ${unsafeCSS(
  ...
  renderNote() {
    if (this.allowCreateDoc$.value) {
      return html` <div>
        <div
          @click="${this.addNote}"
          style="max-width: var(--blank-editor-width);margin: auto;cursor: pointer;color: var(--blank-text-disable-color)"
        >
