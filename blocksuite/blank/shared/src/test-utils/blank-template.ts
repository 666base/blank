...
export function createBlankTemplate(
   ...
   *   <blank-page id="page">
   *     <blank-note id="note">
   *       <blank-paragraph id="paragraph-1">Hello, world<anchor /></blank-paragraph>
   *       <blank-paragraph id="paragraph-2">Hello, world<focus /></blank-paragraph>
   *     </blank-note>
   *   </blank-page>
   * `;
   * ```
   */
  function blank(strings: TemplateStringsArray, ...values: any[]) {
  ...
  function block(
    ...
    doc.load(() => {
      ...
      if (
        ...
        const pageId = store.addBlock('blank:page', {});
        const noteId = store.addBlock('blank:note', {}, pageId);
...
export const { blank, block } = createBlankTemplate();
