...
export const LatexExtension = InlineMarkdownExtension<BlankTextAttributes>({
  name: 'latex',
  ...
  action: ({ inlineEditor, prefixText, inlineRange, pattern, undoManager }) => {
    ...
    const ifEdgelessText = blockComponent.closest('blank-edgeless-text');
    ...
    if (blockPrefix === '$$$$') {
      ...
      blockComponent.host.updateComplete
        .then(() => {
          ...
          if (!latexBlock || latexBlock.flavour !== 'blank:latex') return;
    ...
    if (inlinePrefix === '$$') {
      ...
      inlineEditor
        ...
        .then(async () => {
          ...
          const latexNode = text.parentElement?.closest('blank-latex-node');
          if (!latexNode) return;
