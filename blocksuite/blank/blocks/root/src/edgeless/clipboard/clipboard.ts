import { addAttachments } from '@blocksuite/blank-block-attachment';
import { EdgelessFrameManagerIdentifier } from '@blocksuite/blank-block-frame';
import { addImages } from '@blocksuite/blank-block-image';
import {
  CanvasRenderer,
  DefaultTool,
...
import { splitIntoLines } from '@blocksuite/blank-gfx-text';
import type {
  EmbedCardStyle,
  ShapeElementModel,
...
export class EdgelessClipboardController extends PageClipboard {
  static override key = 'blank-edgeless-clipboard';
  ...
  private readonly _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      // try to interpret url as blank doc url
  ...
  private async _edgelessToCanvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async function (documentClone: Document, element: HTMLElement) {
        ...
        const layer = documentClone.querySelector('.blank-edgeless-layer');
  ...
  private async _pasteTextContentAsNote(content: BlockSnapshot[] | string) {
    ...
    if (typeof content === 'string') {
      ...
      for (let index = 0; index < content.length; index++) {
        ...
        if (blockSnapshot.flavour === 'blank:note') {
