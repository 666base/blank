...
export const sideMap = new Map([
  ...
  ['blank:surface:frame', { top: 28 }],
  // includes group element
  ['blank:surface:group', { top: 20 }],
  // has only one shape element
  ['blank:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'blank:note';
...
export function renderToolbar(
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['blank:surface:*', 'custom:blank:surface:*'] : [],
    'blank:*',
    'custom:blank:*',
  ]
    .flat()
  ...
  // Avoids shaking
  if (flavour === 'blank:note' && context.std.range.value) {
