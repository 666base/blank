import '@blocksuite/blank-shared/commands';

import { CaptionedBlockComponent } from '@blocksuite/blank-components/caption';
import { playCheckAnimation } from '@blocksuite/blank-components/icons';
import { TOGGLE_BUTTON_PARENT_CLASS } from '@blocksuite/blank-components/toggle-button';
import { DefaultInlineManagerExtension } from '@blocksuite/blank-inline-preset';
import type { ListBlockModel } from '@blocksuite/blank-model';
import type { RichText } from '@blocksuite/blank-rich-text';
import {
  BLOCK_CHILDREN_CONTAINER_PADDING_LEFT,
  EDGELESS_TOP_CONTENTEDITABLE_SELECTOR,
} from '@blocksuite/blank-shared/consts';
import { DocModeProvider } from '@blocksuite/blank-shared/services';
import { getViewportElement } from '@blocksuite/blank-shared/utils';
...
export class ListBlockComponent extends CaptionedBlockComponent<ListBlockModel> {
  ...
  private readonly _onClickIcon = (e: MouseEvent) => {
    ...
    if (this.model.props.type === 'toggle') {
      ...
      if (this.model.props.checked) {
        const checkEl = this.querySelector('.blank-list-block__todo-prefix');
        if (checkEl) {
  ...
  override renderBlock(): TemplateResult<1> {
    ...
    const children = html`<div
      class="blank-block-children-container"
      style=${styleMap({
    ...
    return html`
      <div class=${'blank-list-block-container'} style="${textAlignStyle}">
        <div
          class=${classMap({
            'blank-list-rich-text-wrapper': true,
            'blank-list--checked':
  ...
  override accessor blockContainerStyles = {
    margin: 'var(--blank-list-margin, 10px 0)',
  };
}
