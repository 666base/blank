import { NoteBlockModel, RootBlockModel } from '@blocksuite/blank-model';
...
// margin-top: calc(var(--blank-paragraph-space) + 24px);
...
function isImage({ tagName }: Element) {
  return tagName === 'BLANK-IMAGE';
}

function isDatabase({ tagName }: Element) {
  return tagName === 'BLANK-DATABASE';
}
 ...
 * In Chrome/Safari, `document.elementsFromPoint` does not include `blank-image`.
 */
