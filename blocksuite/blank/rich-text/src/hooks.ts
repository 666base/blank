...
import { isStrictUrl } from '@blocksuite/blank-shared/utils';
import type {
  BeforeinputHookCtx,
...
const autoIdentifyLink = (ctx: HookContext<BlankTextAttributes>) => {
...
function handleExtendedAttributes(
  ctx:
    | BeforeinputHookCtx<BlankTextAttributes>
    | CompositionEndHookCtx<BlankTextAttributes>
) {
...
export const onVBeforeinput = (
  ctx: BeforeinputHookCtx<BlankTextAttributes>
) => {
...
export const onVCompositionEnd = (
  ctx: CompositionEndHookCtx<BlankTextAttributes>
) => {
  handleExtendedAttributes(ctx);
};
