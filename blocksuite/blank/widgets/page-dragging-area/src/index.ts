...
import { ViewportElementProvider } from '@blocksuite/blank-shared/services';
import {
  autoScroll,
  getScrollContainer,
...
export const BLANK_PAGE_DRAGGING_AREA_WIDGET =
  'blank-page-dragging-area-widget';

export class BlankPageDraggingAreaWidget extends WidgetComponent<RootBlockModel> {
  static excludeFlavours: string[] = ['blank:note', 'blank:surface'];
    ...
    const getAllNodeFromTree = (): BlockComponent[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !BlankPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
          blocks.push(view);
  ...
  override render() {
    ...
    return html`
      ...
      <div class="blank-page-dragging-area" style=${styleMap(style)}></div>
    `;
  }
...
export const pageDraggingAreaWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(BLANK_PAGE_DRAGGING_AREA_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [BLANK_PAGE_DRAGGING_AREA_WIDGET]: BlankPageDraggingAreaWidget;
  }
}
