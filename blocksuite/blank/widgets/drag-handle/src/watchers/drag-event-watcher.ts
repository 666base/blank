...
} from '@blocksuite/blank-block-embed';
import { ParagraphBlockComponent } from '@blocksuite/blank-block-paragraph';
import { DropIndicator } from '@blocksuite/blank-components/drop-indicator';
import {
...
import { surfaceRefToEmbed } from '@blocksuite/blank-shared/adapters';
import {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'blank:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'blank:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'blank:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'blank:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'blank:surface') {
        ...
        if (block.flavour === 'blank:embed-iframe') {
          ...
          block.flavour === 'blank:attachment' ||
          block.flavour === 'blank:bookmark' ||
          block.flavour.startsWith('blank:embed-')
        ) {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    const groupByParent = groupBy(snapshot.content, block =>
      schema.safeValidate(block.flavour, 'blank:surface')
        ? 'blank:surface'
        : schema.safeValidate(block.flavour, 'blank:page')
          ? 'blank:page'
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['blank:surface']) {
        const content = groupByParent['blank:surface'];
      ...
      if (groupByParent['blank:page']) {
        const content = groupByParent['blank:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'blank:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'blank:embed-linked-doc') {
  ...
  private _makeDropTarget(view: BlockComponent) {
    ...
    if (
      // blank:surface block can't be drop target in any modes
