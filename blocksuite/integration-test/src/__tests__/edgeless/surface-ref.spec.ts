...
describe('basic', () => {
  ...
  test('surface-ref should be rendered in page mode', async () => {
    ...
    expect(
      document.querySelector(
        `blank-surface-ref[data-block-id="${surfaceRefId}"]`
      )
    ).instanceOf(Element);
  });
  ...
  test('content in frame should be rendered in the correct order', async () => {
    ...
    const refBlocks = Array.from(
      surfaceRef.querySelectorAll('blank-edgeless-note')
    ) as HTMLElement[];
...
import type { FrameBlockComponent } from '@blocksuite/blank/blocks/frame';
...
describe('clipboard', () => {
  test('import surface-ref snapshot should render content correctly', async () => {
    ...
    const surfaceRefs = newDoc!.getBlocksByFlavour('blank:surface-ref');
