...
import { useInsidePeekView } from '@blank/core/modules/peek-view/view/modal-container';
...
interface BlankPageReferenceProps {
  pageId: string;
...
function BlankPageReferenceInner({
  pageId,
  params,
  title,
  Icon: UserIcon,
}: BlankPageReferenceProps) {
  ...
  return (
    <span className={notFound ? styles.notFound : ''}>
      ...
      <span className="blank-reference-title">{title}</span>
    </span>
  );
}

export function BlankPageReference({
  pageId,
  params,
  title,
  className,
...
export function BlankSharedPageReference({
  pageId,
  docCollection,
  params,
  title,
